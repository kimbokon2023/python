...
def lcN20():    
    ...
    dim_diameter(doc, (circle11x1, circle11y1), 11, angle=225, dimstyle="JKW", override=None) 
    dim_diameter(doc, (math.floorircle11x2, circle11y2), 11, angle=225, dimstyle="JKW", override=None) 
...
def MainCeiling():   
    ...
    if program:                
        ...
        dim(doc, x1, y1, x1 + 33, y1 + ELYpos, 100, direction="left", localdimstyle="0.2 JKW")#헤다
        #하부 치수선
        textstr =  f"AS = {AS_raw}"
        ...
        #내부 치수선
        dim(doc, x1 + 225, y1 + ELYpos + 30, x1, y1 + ELYpos + 30, 100, direction="up", option="reverse", localdimstyle="0.2 JKW")#헤더 위 가로 브라켓
    ...
    ###################################################
        ...
        for i, x in enumerate(Bottomx):
            startX = x1 - 6
            if i != 0:
                # 첫 번째 선: (x12, y12) -> (x1 - 17.5, y12)
                # line(doc, x12, y12, x12 + x - 17.5, y12)            
                # 나머지 선: (x + 17.5, y12) -> (다음 x - 17.5, y12)
                line(doc, startX + Bottomx[i-1] + 17.5, y12, startX + x - 17.5, y12)
