...
def main():
    ...
    for file_path in xlsm_files:
        ...
        # A열 데이터 (1열)
        column_a_data = []
        start_y = 500  # 텍스트를 그릴 시작 y좌표
        line_spacing = 20 # 한 줄 간격

        for row in range(1, max_row + 1):
            value = sheet.cell(row=row, column=1).value
            column_a_data.append(value)
            
            # 도면에 텍스트를 추가할 위치 설정
            text_x = 100
            text_y = start_y - (row - 1) * line_spacing
            
            # 엑셀 값을 문자열로 변환하여 텍스트 그리기
            if value is not None:
                draw_Text(doc, text_x, text_y, 20, str(value), layer='0') 
                
        # B열 데이터 (2열)
        column_b_data = []
        for row in range(1, max_row + 1):
            value = sheet.cell(row=row, column=2).value
            column_b_data.append(value)
            
        start_y = 500  # 텍스트를 시작할 y좌표
        line_spacing = 20 # 한 줄 간격

        for i in range(len(column_a_data)):
            # A열 데이터를 (100, 500), (100, 480)... 위치에 배치
            msp.add_text(column_a_data[i], dxfattribs={'insert': (100, start_y - i * line_spacing)})
            
            # B열 데이터를 (300, 500), (300, 480)... 위치에 배치
            msp.add_text(column_b_data[i], dxfattribs={'insert': (300, start_y - i * line_spacing)})
