...
# PyInstaller exe 환경에서 buffer 속성이 없을 수 있으므로 체크
...
ELYpos = 0
footOP = 0
headertype, emergencytype, LHtype = None, None, None
...
# ceiling_excel 폴더는 exe 실행 위치에서 찾기 (외부 폴더)
if getattr(sys, '_MEIPASS', None):
    # exe로 실행 중일 때 - 현재 작업 디렉토리에서 ceiling_excel 찾기
    excel_saved_file = os.path.join(os.getcwd(), 'ceiling_excel')
else:
    # 개발 환경에서는 스크립트와 같은 위치
    excel_saved_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ceiling_excel')
...
# DXF 파일 로드 - PyInstaller exe 환경과 개발 환경 모두 처리
if getattr(sys, '_MEIPASS', None):
    block_path = os.path.join(application_path, 'block.dxf')
else:
    block_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'block.dxf')
doc = ezdxf.readfile(block_path)
msp = doc.modelspace()
...
def lc026():     
    ...
    if LC_height != 100 :
        if LCframeMaterial in ['', 'SPCC 1T', 'SPCC 1.2T']:
            insert_block(rx1, ry1, "lc026_frame_section_1T")
        else:    
            insert_block(rx1, ry1, "lc026_frame_section")    
    else:
        if LCframeMaterial in ['', 'SPCC 1T', 'SPCC 1.2T']:
            insert_block(rx1 - 593.2, ry1, "lc026_frame_section_1T")
        else:    
    ...
    if LCframeMaterial in ['', 'SPCC 1T']:
        textstr = f"Size : {LCD} x 491.5"    
    elif LCframeMaterial == 'SPCC 1.6T' :
        textstr = f"Size : {LCD} x 524"    
    elif LCframeMaterial == 'SPCC 1.2T' :
    ...
    # 상부 치수선
    dim(doc, circle70x1, circle70y1, x12, y12, 412, text_height=0.22, text_gap=0.07, direction="up", option="reverse")
    ...
    dim(doc, circle20x1, circle20y1, x12, y12, 285, text_height=0.22, text_gap=0.07, direction="up", option="reverse")
    dim(doc, circle20x1, circle20y1, circle20x2, circle20y2, 285, text_height=0.22, text_gap=0.07, direction="up")
    dim(doc, circle20x2, circle20y2, x11, y11, 285, text_height=0.22, text_gap=0.07, direction="up")
    dim(doc, x12, y12, x11, y11, 263, text_height=0.22, text_gap=0.07, direction="up")
...
def lc035():   
    ...
    # 5page 중판 (뒷쪽) 
    #################################################################################################        
    ##### 상하 대칭으로 코드 수정함 부호만 바꿈 (2025/04/09)
    program = 1
    if program:         
        # 도면틀 넣기
        BasicXscale = 2813
...
def MainCeiling():   
    ...
    global SplitYpos1, SplitYpos2, SplitYpos3, MDFirstSpace, WGap, MDCenterBK, MDSecondSpace, entrytype
    ...
    if program:                
        ...
        def common_cal(CD, CW, ceilingdivision, ELYpos, sidePanel):
            ...
            if CD <= 1200 or ceilingdivision == 1:
                SplitYpos1 = MCD
                SplitYpos2 = 0
                SplitYpos3 = 0
                MDFirstSpace = math.floor((MCD - ELYpos - 130) / 4)
                MDSecondSpace = 0
                MDCenterBK = 0
        ...
        if headertype == "세화정공":
            temp_x1 = x2
            temp_y1 = y1
            temp_x2 = temp_x1 - (398 + (OP_raw - 900) * 0.5)
            temp_y2 = temp_y1
            temp_x3 = temp_x2
            temp_y3 = temp_y2 + 25
            temp_x4 = temp_x3 - 158
            ...
            temp_y6 = temp_y5
            temp_x7 = temp_x6
            temp_y7 = temp_y6 + (MCD - SplitYpos1)
            temp_x8 = temp_x7 + MCW
            temp_y8 = temp_y7
            ...
            for i in range(1, lastNum + 1):
                ...
                line(doc, prev_x, prev_y, curr_x, curr_y, layer="0")
                prev_x, prev_y = curr_x, curr_y
            line(doc, prev_x, prev_y, temp_x1, temp_y1, layer="0")  
            
            dim(doc, temp_x5, temp_y5, temp_x4, temp_y4, 50, direction="right", localdimstyle="0.2 JKW")
            dim(doc, temp_x6, temp_y6, temp_x5, temp_y5, 50, direction="down", localdimstyle="0.2 JKW")
            dimcontinue(doc, temp_x2, temp_y2)
            dimcontinue(doc, temp_x1, temp_y1)
        else:
            rectangle(doc, x1, y1, x2, y1 + (MCD - SplitYpos1), layer='0')
        
        if entrytype == "관통":
            if headertype == "세화정공": #상부 따임
                temp_x1 = x1
                temp_y1 = y2
                temp_x2 = temp_x1 + (398 + (OP_raw - 900) * 0.5)
                temp_y2 = temp_y1
                ...
                dim(doc, temp_x5, temp_y5, temp_x4, temp_y4, 50, direction="left", localdimstyle="0.2 JKW")
                dim(doc, temp_x6, temp_y6, temp_x5, temp_y5, 50, direction="up", localdimstyle="0.2 JKW")
                dimcontinue(doc, temp_x2, temp_y2)
                dimcontinue(doc, temp_x1, temp_y1)
            else:
        ...
        for x in frontPanel[:-1] :
            ...
            dim(doc, x1 + sum, y1, x1 + sum - x, y1, 120, direction="down", localdimstyle="0.2 JKW")    
        ...
        for x in rearPanel[:-1] :
            ...
            dim(doc, x1 + sum, y2, x1 + sum - x, y2, 120, direction="up", localdimstyle="0.2 JKW")    
        ...
        if lctype == "035":
            ...
            for i, x in enumerate(Topholex):
                ...
                if i == 0:
                    dim(doc, x1 + 25 + x, y2 - 60, x1, y2, 230, direction="up", option='reverse', localdimstyle="0.2 JKW")    
                else:
            ...
            if headertype == "이노에스템":
                for i, x in enumerate(Bottomx):
                    ...
                    if i == 0:
                        dim(doc, x1 + 25 + x, y1 + 50, x1, y1, 240, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    else:
                        dimcontinue(doc, x1 + x + 25, y1 + 50)
                dimcontinue(doc, x2, y1) 
            elif headertype in ["에이스코리아", "세화정공"]:
                for i, x in enumerate(Bottomx):
            ...
            for i, x in enumerate(Topholex):
                ...
                if i == 0:
                    dim(doc, x1 + 25 + x, y2 - 90, x1, y2, 260, direction="up", option='reverse', localdimstyle="0.2 JKW")    
                else:
            ...
            if headertype == "세화정공":
                for x in Bottomx:
                    insert_block(x1 + 25 + x, y1 + 100, "Ceiling_LChole_sehwa_008A")
            elif headertype == "에이스코리아":
                for x in Bottomx:
                    insert_block(x1 + 25 + x, y1 + 100, "Ceiling_LChole_ace_008A")
            else:
                for x in Bottomx:
            ...
            dim(doc, x1 + 137.5, y2 - 60, x1, y2, 230, direction="up", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 137.5 + 185, y2 - 60)
            dimcontinue(doc, x2 - (137.5 + 185), y2 - 60)
            dimcontinue(doc, x2 - 137.5, y2 - 60)
            dimcontinue(doc, x2, y2)
            dim(doc, x1 + 137.5, y1 + 60, x1, y1, 240, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            ...
            dim(doc, x1 + 170, y2 - 60, x1, y2, 230, direction="up", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 170 + 150, y2 - 60)
            dimcontinue(doc, x2 - (170 + 150), y2 - 60)
            dimcontinue(doc, x2 - 170, y2 - 60)
            dimcontinue(doc, x2, y2)
            dim(doc, x1 + 170, y1 + 60, x1, y1, 240, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            ...
            circle_cross(doc, x1 + 165, y2 - 65, 13, layer='0')#하
            circle_cross(doc, x1 + 165 + 160, y2 - 65, 13, layer='0')
            circle_cross(doc, x2 - 165, y2 - 65, 13, layer='0')#상
            circle_cross(doc, x2 - 165 - 160, y2 - 65, 13, layer='0')
            circle_cross(doc, x1 + 125, y1 + MCD/2, 13, layer='0')#중간
            circle_cross(doc, x2 - 125, y1 + MCD/2, 13, layer='0')
            
            dim(doc, x1 + 165, y2 - 60, x1, y2, 230, direction="up", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 165 + 160, y2 - 60)
            dimcontinue(doc, x2 - (165 + 160), y2 - 60)
            dimcontinue(doc, x2 - 165, y2 - 60)
            dimcontinue(doc, x2, y2)
            dim(doc, x1 + 165, y1 + 60, x1, y1, 240, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 165 + 160, y1 + 60)
            dimcontinue(doc, x2 - (165 + 160), y1 + 60)
            dimcontinue(doc, x2 - 165, y1 + 60)
            dimcontinue(doc, x2, y1)
            dim(doc, x2 - 160, y1 + 65, x2, y1, 360, direction="right", option='reverse', localdimstyle="0.2 JKW")
            dimcontinue(doc, x2 - 120, y1 + MCD/2)
            dimcontinue(doc, x2 - 160, y2 - 65)
            dimcontinue(doc, x2, y2)
        ...
        elif lctype in ("011", "017", "025"):
            if headertype == "이노에스템":
                insert_block(x1 + 155, y1 + 65, "Ceiling_LChole_ino")
                insert_block(x1 + 155 + 160, y1 + 65, "Ceiling_LChole_ino")
                insert_block(x2 - 155, y1 + 65, "Ceiling_LChole_ino")
                insert_block(x2 - 155 - 160, y1 + 65, "Ceiling_LChole_ino")
            elif headertype in ["에이스코리아", "세화정공"]:
                insert_block(x1 + 155, y1 + 65, "Ceiling_LChole_sehwa")
                insert_block(x1 + 155 + 160, y1 + 65, "Ceiling_LChole_sehwa")
                insert_block(x2 - 155, y1 + 65, "Ceiling_LChole_sehwa")
                insert_block(x2 - 155 - 160, y1 + 65, "Ceiling_LChole_sehwa")
            else:
                circle_cross(doc, x1 + 155, y1 + 65, 13, layer='0')
                circle_cross(doc, x1 + 155 + 160, y1 + 65, 13, layer='0')
                circle_cross(doc, x2 - 155, y1 + 65, 13, layer='0')
                circle_cross(doc, x2 - 155 - 160, y1 + 65, 13, layer='0')
                
            circle_cross(doc, x1 + 155, y2 - 65, 13, layer='0')
            circle_cross(doc, x1 + 155 + 160, y2 - 65, 13, layer='0')
            circle_cross(doc, x2 - 155, y2 - 65, 13, layer='0')
            circle_cross(doc, x2 - 155 - 160, y2 - 65, 13, layer='0')
            
            dim(doc, x1 + 155, y2 - 60, x1, y2, 230, direction="up", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 155 + 160, y2 - 60)
            dimcontinue(doc, x2 - (155 + 160), y2 - 60)
            dimcontinue(doc, x2 - 155, y2 - 60)
            dimcontinue(doc, x2, y2)
            dim(doc, x1 + 155, y1 + 60, x1, y1, 240, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 155 + 160, y1 + 60)
            dimcontinue(doc, x2 - (155 + 160), y1 + 60)
            dimcontinue(doc, x2 - 155, y1 + 60)
            dimcontinue(doc, x2, y1)
            dim(doc, x2 - 160, y1 + 65, x2, y1, 360, direction="right", option='reverse', localdimstyle="0.2 JKW")
        ...
        elif lctype in ("013", "013D"):
            if headertype == "이노에스템":
                insert_block(x1 + 162, y1 + 65, "Ceiling_LChole_ino")
                insert_block(x1 + 162 + 130, y1 + 65, "Ceiling_LChole_ino")
                insert_block(x2 - 162, y1 + 65, "Ceiling_LChole_ino")
                insert_block(x2 - 162 - 130, y1 + 65, "Ceiling_LChole_ino")
            elif headertype in ["에이스코리아", "세화정공"]:
                insert_block(x1 + 162, y1 + 65, "Ceiling_LChole_sehwa")
                insert_block(x1 + 162 + 130, y1 + 65, "Ceiling_LChole_sehwa")
                insert_block(x2 - 162, y1 + 65, "Ceiling_LChole_sehwa")
                insert_block(x2 - 162 - 130, y1 + 65, "Ceiling_LChole_sehwa")
            else:
                circle_cross(doc, x1 + 162, y1 + 65, 13, layer='0')
                circle_cross(doc, x1 + 162 + 130, y1 + 65, 13, layer='0')
                circle_cross(doc, x2 - 162, y1 + 65, 13, layer='0')
                circle_cross(doc, x2 - 162 - 130, y1 + 65, 13, layer='0')
                
            circle_cross(doc, x1 + 162, y2 - 65, 13, layer='0')
            circle_cross(doc, x1 + 162 + 130, y2 - 65, 13, layer='0')
            circle_cross(doc, x2 - 162, y2 - 65, 13, layer='0')
            circle_cross(doc, x2 - 162 - 130, y2 - 65, 13, layer='0')
            
            dim(doc, x1 + 162, y2 - 60, x1, y2, 230, direction="up", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 162 + 130, y2 - 60)
            dimcontinue(doc, x2 - (162 + 130), y2 - 60)
            dimcontinue(doc, x2 - 162, y2 - 60)
            dimcontinue(doc, x2, y2)
            dim(doc, x1 + 162, y1 + 60, x1, y1, 240, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 162 + 130, y1 + 60)
            dimcontinue(doc, x2 - (162 + 130), y1 + 60)
            dimcontinue(doc, x2 - 162, y1 + 60)
            dimcontinue(doc, x2, y1)
            dim(doc, x2 - 160, y1 + 65, x2, y1, 360, direction="right", option='reverse', localdimstyle="0.2 JKW")
            dimcontinue(doc, x2 - 160, y2 - 65)
            dimcontinue(doc, x2, y2)
        ...
        if entrytype == "관통":
            if headertype == "이노에스템":
                rectangle(doc, x1 + 31, y2 - 32.5, x2 - 31, y2 - (32.5 + 55), layer='4')
                rectangle(doc, x1 + 76, y2 - 32.5, x2 - 76, y2 - (32.5 + 15), layer='4')
                rectangle(doc, x1 + 76, y2 - (32.5 + 40), x2 - 76, y2 - (32.5 + 15 + 40), layer='4')
                rectangle(doc, x1 + 31, y2 - 107.5, x2 - 31, y2 - (107.5 + 55), layer='4')
                rectangle(doc, x1 + 76, y2 - 107.5, x2 - 76, y2 - (107.5 + 15), layer='4')
                rectangle(doc, x1 + 76, y2 - (107.5 + 40), x2 - 76, y2 - (107.5 + 55), layer='4')
            elif headertype == "정일산업":
                rectangle(doc, x1 + 31, y2 - 30, x2 - 31, y2 - (30 + 75), layer='4')
                rectangle(doc, x1 + 76, y2 - 30, x2 - 76, y2 - (30 + 15), layer='4')
                rectangle(doc, x1 + 76, y2 - (30 + 60), x2 - 76, y2 - (30 + 15 + 60), layer='4')
                rectangle(doc, x1 + 31, y2 - 105, x2 - 31, y2 - (105 + 75), layer='4')
                rectangle(doc, x1 + 76, y2 - 105, x2 - 76, y2 - (105 + 15), layer='4')
                rectangle(doc, x1 + 76, y2 - (105 + 60), x2 - 76, y2 - (105 + 15 + 60), layer='4')
            elif headertype == "에이스코리아":
                rectangle(doc, x1 + 31, y2 - 30, x2 - 31, y2 - (30 + 55), layer='4')
                rectangle(doc, x1 + 76, y2 - 30, x2 - 76, y2 - (30 + 15), layer='4')
                rectangle(doc, x1 + 76, y2 - (30 + 40), x2 - 76, y2 - (30 + 15 + 40), layer='4')
                rectangle(doc, x1 + 31, y2 - 85, x2 - 31, y2 - (85 + 55), layer='4')
                rectangle(doc, x1 + 76, y2 - 85, x2 - 76, y2 - (85 + 15), layer='4')
                rectangle(doc, x1 + 76, y2 - (85 + 40), x2 - 76, y2 - (85 + 15 + 40), layer='4')
            elif headertype == "세화정공":
                rectangle(doc, x1 + 31, y2 - 35, x2 - 31, y2 - (35 + 100), layer='4')
                line(doc, x1 + 31, y2 - (35 + 13), x2 - 31, y2 - (35 + 13), layer="4")
                line(doc, x1 + 31, y2 - (35 + 50), x2 - 31, y2 - (35 + 50), layer="4")
                line(doc, x1 + 31, y2 - (35 + 87), x2 - 31, y2 - (35 + 87), layer="4")
                line(doc, x1 + 31, y2 - (35 + 2.3), x2 - 31, y2 - (35 + 2.3), layer="Hidden")
                line(doc, x1 + 31, y2 - (35 + 2.3 + 45.4), x2 - 31, y2 - (35 + 2.3 + 45.4), layer="Hidden")
                line(doc, x1 + 31, y2 - (35 + 2.3 + 50), x2 - 31, y2 - (35 + 2.3 + 50), layer="Hidden")
                line(doc, x1 + 31, y2 - (35 + 2.3 + 95.4), x2 - 31, y2 - (35 + 2.3 + 95.4), layer="Hidden")
        ...
        if emergencytype == "있음":
            if entrytype == "관통":
                ...
                eme_through = 0
            EXITx1 = x1 + MCW/2 - emergencyRow/2
            EXITx2 = x1 + MCW/2 + emergencyRow/2
            EXITy1 = y2 - 150 - emergencyCol - eme_through
            EXITy2 = y2 - 150 - eme_through
            
            rectangle(doc, EXITx1, EXITy1, EXITx2, EXITy2, layer='6')
            rectangle(doc, EXITx1 - 10, EXITy1 - 10, EXITx2 + 10, EXITy2 + 10, layer='0')
            line(doc, EXITx1, EXITy1, EXITx2, EXITy2, layer='0중심')
            line(doc, EXITx1, EXITy2, EXITx2, EXITy1, layer='0중심')
            
            line(doc, EXITx1 - 10, EXITy1 + emergencyCol/2 - 50, EXITx2 + 10, EXITy1 + emergencyCol/2 - 50, layer='0')
            line(doc, EXITx1 - 10, EXITy1 + emergencyCol/2 - 50 + 20, EXITx2 + 10, EXITy1 + emergencyCol/2 - 50 + 20, layer='0')
            line(doc, EXITx1 - 10, EXITy1 + emergencyCol/2 - 50 - 20, EXITx2 + 10, EXITy1 + emergencyCol/2 - 50 - 20, layer='0')
            line(doc, EXITx1 - 10, EXITy1 + emergencyCol/2 - 50 + 1.6, EXITx2 + 10, EXITy1 + emergencyCol/2 - 50 + 1.6, layer='2')
            line(doc, EXITx1 - 10, EXITy1 + emergencyCol/2 - 50 - 1.6, EXITx2 + 10, EXITy1 + emergencyCol/2 - 50 - 1.6, layer='2')
            
            insert_block(EXITx1 + 100, EXITy1 + emergencyCol/2 - 50, "Ceiling_exit_hinge")
            insert_block(EXITx2 - 100, EXITy1 + emergencyCol/2 - 50, "Ceiling_exit_hinge")
            insert_block(EXITx1 + emergencyRow/2, EXITy1, "Ceiling_exit_switch")
            
            #비상구 상하 브라켓
            rectangle(doc, EXITx1 - 44, EXITy1 - 83, EXITx2 + 44, EXITy1 - 18, layer='0')
            rectangle(doc, EXITx1 - 44, EXITy1 - 80.7, EXITx2 + 44, EXITy1 - 20.3, layer='0')
            rectangle(doc, EXITx1 - 44, EXITy2 + 11, EXITx2 + 44, EXITy2 + 61, layer='0')
            line(doc, EXITx1 - 44, EXITy2 + 61 - 2.3, EXITx2 + 44, EXITy2 + 61 - 2.3, layer='0')
            
            dim(doc, EXITx1, EXITy2 - 100, EXITx2, EXITy2 - 100, 50, direction="down", localdimstyle="0.2 JKW")
            dim(doc, x1 + MCW/2, y2, x1 + MCW/2, y2 - 150 - eme_through, 150, direction="right", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + MCW/2, EXITy1)
            
            #비상구 하부 세로 긴 브라켓 길이 저장
            EXsizey1 = EXITy1 - 83.5 - (y1 + ELYpos + 65)
            
            if CW<1700:
                #좌 세로 브라켓
                rectangle(doc, EXITx1 - 144, y1 + ELYpos + 66, EXITx1 - 44, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx1 - 144 + 18, y1 + ELYpos + 66, EXITx1 - 44 - 18, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx1 - 144 + 16, y1 + ELYpos + 66, EXITx1 - 44 - 16, y2 - 30.5 - through, layer='2')
                insert_block(EXITx1 - 94, y1 + ELYpos + 96, "Ceiling_Bracket_vertical_hole2_100")
                insert_block(EXITx1 - 94, y2 - 60.5 - through, "Ceiling_Bracket_vertical_hole2_100")
                insert_block(EXITx1 - 94, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)/3, "Ceiling_Bracket_vertical_hole4_100")
                insert_block(EXITx1 - 94, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)*2/3, "Ceiling_Bracket_vertical_hole4_100")

                #우 세로 브라켓
                rectangle(doc, EXITx2 + 144, y1 + ELYpos + 66, EXITx2 + 44, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx2 + 144 - 18, y1 + ELYpos + 66, EXITx2 + 44 + 18, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx2 + 144 - 16, y1 + ELYpos + 66, EXITx2 + 44 + 16, y2 - 30.5 - through, layer='2')
                insert_block(EXITx2 + 94, y1 + ELYpos + 96, "Ceiling_Bracket_vertical_hole2_100")
                insert_block(EXITx2 + 94, y2 - 60.5 - through, "Ceiling_Bracket_vertical_hole2_100")
                insert_block(EXITx2 + 94, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5))/3, "Ceiling_Bracket_vertical_hole4_100")
                insert_block(EXITx2 + 94, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5))*2/3, "Ceiling_Bracket_vertical_hole4_100")
                
                #좌측 가로 브라켓
                rectangle(doc, x1 + 30.5, y1 + (MCD - SplitYpos1) - 15, EXITx1 - 144 - 0.5, y1 + (MCD - SplitYpos1) + 15, layer='2')
                line(doc, x1 + 30.5, y1 + (MCD - SplitYpos1) + 13, EXITx1 - 144 - 0.5, y1 + (MCD - SplitYpos1) + 13, layer="2")
                rectangle(doc, x1 + 30.5, y2 - 700 - 15, EXITx1 - 144 - 0.5, y2 - 700 + 15, layer='2')
                line(doc, x1 + 30.5, y2 - 700 + 13, EXITx1 - 144 - 0.5, y2 - 700 + 13, layer="2")
                
                #우측 가로 브라켓
                rectangle(doc, x2 - 30.5, y1 + (MCD - SplitYpos1) - 15, EXITx2 + 144 + 0.5, y1 + (MCD - SplitYpos1) + 15, layer='2')
                line(doc, x2 - 30.5, y1 + (MCD - SplitYpos1) + 13, EXITx2 + 144 + 0.5, y1 + (MCD - SplitYpos1) + 13, layer="2")
                rectangle(doc, x2 - 30.5, y2 - 700 - 15, EXITx2 + 144 + 0.5, y2 - 700 + 15, layer='2')
                line(doc, x2 - 30.5, y2 - 700 + 13, EXITx2 + 144 + 0.5, y2 - 700 + 13, layer="2")
                
                #상부 치수선  
                dim(doc, x1, y2, x2, y2, 260, direction="up", localdimstyle="0.2 JKW")#상부 전체
                dim(doc, x1 + Xgap, y2 - Ygap, x1, y2, 260, text_height=0.32, text_gap=0.07, direction="up", option="reverse", localdimstyle="0.2 JKW")
                dimcontinue(doc, EXITx1 - 144, y2 - Ygap)
                dimcontinue(doc, EXITx1 - 44, y2 - Ygap)
                dimcontinue(doc, EXITx2 + 44, y2 - Ygap)
                dimcontinue(doc, EXITx2 + 144, y2 - Ygap)
                dimcontinue(doc, x2 - Xgap, y2 - Ygap)
                dimcontinue(doc, x2, y2)
                
                #비상구 좌우 가로 브라켓 길이 저장
                EXsizex1 = EXITx1 - 144 - 0.5 - (x1 + 30.5)
            
            else: #CW>=1700
                #좌 세로 브라켓
                rectangle(doc, EXITx1 - 174, y1 + ELYpos + 66, EXITx1 - 44, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx1 - 174 + 18, y1 + ELYpos + 66, EXITx1 - 44 - 18, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx1 - 174 + 16, y1 + ELYpos + 66, EXITx1 - 44 - 16, y2 - 30.5 - through, layer='2')
                insert_block(EXITx1 - 109, y1 + ELYpos + 96, "Ceiling_Bracket_vertical_hole2")
                insert_block(EXITx1 - 109, y2 - 60.5 - through, "Ceiling_Bracket_vertical_hole2")
                insert_block(EXITx1 - 109, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)/3, "Ceiling_Bracket_vertical_hole4")
                insert_block(EXITx1 - 109, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)*2/3, "Ceiling_Bracket_vertical_hole4")

                #우 세로 브라켓
                rectangle(doc, EXITx2 + 174, y1 + ELYpos + 66, EXITx2 + 44, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx2 + 174 - 18, y1 + ELYpos + 66, EXITx2 + 44 + 18, y2 - 30.5 - through, layer='2')
                rectangle(doc, EXITx2 + 174 - 16, y1 + ELYpos + 66, EXITx2 + 44 + 16, y2 - 30.5 - through, layer='2')
                insert_block(EXITx2 + 109, y1 + ELYpos + 96, "Ceiling_Bracket_vertical_hole2")
                insert_block(EXITx2 + 109, y2 - 60.5 - through, "Ceiling_Bracket_vertical_hole2")
                insert_block(EXITx2 + 109, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5))/3, "Ceiling_Bracket_vertical_hole4")
                insert_block(EXITx2 + 109, y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5))*2/3, "Ceiling_Bracket_vertical_hole4")
                
                #좌측 가로 브라켓
                rectangle(doc, x1 + 30.5, y1 + (MCD - SplitYpos1) - 15, EXITx1 - 174 - 0.5, y1 + (MCD - SplitYpos1) + 15, layer='2')
                line(doc, x1 + 30.5, y1 + (MCD - SplitYpos1) + 13, EXITx1 - 174 - 0.5, y1 + (MCD - SplitYpos1) + 13, layer="2")
                rectangle(doc, x1 + 30.5, y2 - 700 - 15, EXITx1 - 174 - 0.5, y2 - 700 + 15, layer='2')
                line(doc, x1 + 30.5, y2 - 700 + 13, EXITx1 - 174 - 0.5, y2 - 700 + 13, layer="2")
                
                #우측 가로 브라켓
                rectangle(doc, x2 - 30.5, y1 + (MCD - SplitYpos1) - 15, EXITx2 + 174 + 0.5, y1 + (MCD - SplitYpos1) + 15, layer='2')
                line(doc, x2 - 30.5, y1 + (MCD - SplitYpos1) + 13, EXITx2 + 174 + 0.5, y1 + (MCD - SplitYpos1) + 13, layer="2")
                rectangle(doc, x2 - 30.5, y2 - 700 - 15, EXITx2 + 174 + 0.5, y2 - 700 + 15, layer='2')
                line(doc, x2 - 30.5, y2 - 700 + 13, EXITx2 + 174 + 0.5, y2 - 700 + 13, layer="2")
                
                #상부 치수선  
                dim(doc, x1, y2, x2, y2, 280, direction="up", localdimstyle="0.2 JKW")#상부 전체
                dim(doc, x1 + Xgap, y2 - Ygap, x1, y2, 260, text_height=0.32, text_gap=0.07, direction="up", option="reverse", localdimstyle="0.2 JKW")
                dimcontinue(doc, EXITx1 - 174, y2 - Ygap)
                dimcontinue(doc, EXITx1 - 44, y2 - Ygap)
                dimcontinue(doc, EXITx2 + 44, y2 - Ygap)
                dimcontinue(doc, EXITx2 + 174, y2 - Ygap)
                dimcontinue(doc, x2 - Xgap, y2 - Ygap)
                dimcontinue(doc, x2, y2)
                
                #비상구 좌우 가로 브라켓 길이 저장
                EXsizex1 = EXITx1 - 174 - 0.5 - (x1 + 30.5)
            
            #비상구 하부 가로 브라켓 위치 저장 (물빠짐 사각홀위치)
            EXsizex3 = y1 + (MCD - SplitYpos1) - (y1 + ELYpos + 66)
            
            #비상구 하부 세로 긴 브라켓
            rectangle(doc, x1 + MCW/2 - 65, y1 + ELYpos + 66, x1 + MCW/2 + 65, EXITy1 - 83.5, layer='2')
            rectangle(doc, x1 + MCW/2 - 65 + 18, y1 + ELYpos + 66, x1 + MCW/2 + 65 - 18, EXITy1 - 83.5, layer='2')
            rectangle(doc, x1 + MCW/2 - 65 + 16, y1 + ELYpos + 66, x1 + MCW/2 + 65 - 16, EXITy1 - 83.5, layer='2')
            insert_block(x1 + MCW/2, y1 + ELYpos + 96, "Ceiling_Bracket_vertical_hole2")
            insert_block(x1 + MCW/2, EXITy1 - 113.5, "Ceiling_Bracket_vertical_hole2")
            if CD>1600:
                insert_block(x1 + MCW/2, y1 + ELYpos + 65.5 + EXsizey1/2, "Ceiling_Bracket_vertical_hole4")
            
            #비상구 하부 세로 긴 브라켓 길이 저장
            EXsizey2 = EXITy1 - 83.5 - (y1 + ELYpos + 66)
            
            #비상구 하부 가로 브라켓 좌
            rectangle(doc, EXITx1 - 43.5, y1 + (MCD - SplitYpos1) - 65, x1 + MCW/2 - 65, y1 + (MCD - SplitYpos1) + 65, layer='2')
            rectangle(doc, EXITx1 - 43.5, y1 + (MCD - SplitYpos1) - 65 + 18, x1 + MCW/2 - 65, y1 + (MCD - SplitYpos1) + 65 - 18, layer='2')
            rectangle(doc, EXITx1 - 43.5, y1 + (MCD - SplitYpos1) - 65 + 16, x1 + MCW/2 - 65, y1 + (MCD - SplitYpos1) + 65 - 16, layer='2')
            insert_block(EXITx1 - 43.5 + 30, y1 + (MCD - SplitYpos1), "Ceiling_Bracket_horizontal_hole2")
            insert_block(x1 + MCW/2 - 65 - 30, y1 + (MCD - SplitYpos1), "Ceiling_Bracket_horizontal_hole2")
            
            #비상구 하부 좌우 가로 브라켓 길이 저장
            EXsizex2 = x1 + MCW/2 - 65 - (EXITx1 - 43.5)
            
            #비상구 하부 가로 브라켓 우
            rectangle(doc, EXITx2 + 43.5, y1 + (MCD - SplitYpos1) - 65, x2 - MCW/2 + 65, y1 + (MCD - SplitYpos1) + 65, layer='2')
            rectangle(doc, EXITx2 + 43.5, y1 + (MCD - SplitYpos1) - 65 + 18, x2 - MCW/2 + 65, y1 + (MCD - SplitYpos1) + 65 - 18, layer='2')
            rectangle(doc, EXITx2 + 43.5, y1 + (MCD - SplitYpos1) - 65 + 16, x2 - MCW/2 + 65, y1 + (MCD - SplitYpos1) + 65 - 16, layer='2')
            insert_block(EXITx2 + 43.5 - 30, y1 + (MCD - SplitYpos1), "Ceiling_Bracket_horizontal_hole2")
            insert_block(x2 - MCW/2 + 65 + 30, y1 + (MCD - SplitYpos1), "Ceiling_Bracket_horizontal_hole2")
        
            #비상구 하부 브라켓 치수선  
            dim(doc, x1 + MCW/2 + 200, y1 + ELYpos + 66, x1 + MCW/2 + 200, EXITy1 - 83.5, 50, direction="left", localdimstyle="0.2 JKW")
            dim(doc, x1 + MCW/2 - 200, y1 + ELYpos + 66, x1 + MCW/2 - 200, y1 + (MCD - SplitYpos1) - 65, 50, text_height=0.32, text_gap=0.07, direction="left", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + MCW/2 - 200, y1 + (MCD - SplitYpos1) + 65)
            dimcontinue(doc, x1 + MCW/2 - 200, EXITy1 - 83.5)
                
        else:
            #좌 세로 브라켓
            rectangle(doc, x1 + (MCW/3 - 130), y1 + ELYpos + 66, x1 + MCW/3, y2 - 30.5 - through, layer='2')
            rectangle(doc, x1 + (MCW/3 - 130) + 18, y1 + ELYpos + 66, x1 + MCW/3 - 18, y2 - 30.5 - through, layer='2')
            rectangle(doc, x1 + (MCW/3 - 130) + 16, y1 + ELYpos + 66, x1 + MCW/3 - 16, y2 - 30.5 - through, layer='Hidden')
            insert_block(x1 + (MCW/3 - 65), y1 + ELYpos + 96, "Ceiling_Bracket_vertical_hole2")
            insert_block(x1 + (MCW/3 - 65), y2 - 60.5 - through, "Ceiling_Bracket_vertical_hole2")
            insert_block(x1 + (MCW/3 - 65), y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)/3, "Ceiling_Bracket_vertical_hole4")
            insert_block(x1 + (MCW/3 - 65), y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)*2/3, "Ceiling_Bracket_vertical_hole4")
            #우 세로 브라켓
            rectangle(doc, x1 + MCW*2/3, y1 + ELYpos + 66, x1 + (MCW*2/3 + 130), y2 - 30.5 - through, layer='2')
            rectangle(doc, x1 + MCW*2/3 + 18, y1 + ELYpos + 66, x1 + (MCW*2/3 + 130) - 18, y2 - 30.5 - through, layer='2')
            rectangle(doc, x1 + MCW*2/3 + 16, y1 + ELYpos + 66, x1 + (MCW*2/3 + 130) - 16, y2 - 30.5 - through, layer='Hidden')
            insert_block(x1 + (MCW*2/3 + 65), y1 + ELYpos + 96, "Ceiling_Bracket_vertical_hole2")
            insert_block(x1 + (MCW*2/3 + 65), y2 - 60.5 - through, "Ceiling_Bracket_vertical_hole2")
            insert_block(x1 + (MCW*2/3 + 65), y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)/3, "Ceiling_Bracket_vertical_hole4")
            insert_block(x1 + (MCW*2/3 + 65), y1 + ELYpos + 66 + (MCD - (ELYpos + 96.5) - through)*2/3, "Ceiling_Bracket_vertical_hole4")
                
            #중간 가로 브라켓 상
            rectangle(doc, x1 + MCW/3 + 0.5, y2 - 30 - MDFirstSpace - 130 - through, x1 + MCW*2/3 - 0.5, y2 - 30 - MDFirstSpace - through, layer='2')
            rectangle(doc, x1 + MCW/3 + 0.5, y2 - 30 - MDFirstSpace - 130 + 18 - through, x1 + MCW*2/3 - 0.5, y2 - 30 - MDFirstSpace - 130 + 112 - through, layer='2')
            rectangle(doc, x1 + MCW/3 + 0.5, y2 - 30 - MDFirstSpace - 130 + 16 - through, x1 + MCW*2/3 - 0.5, y2 - 30 - MDFirstSpace - 130 + 114 - through, layer='Hidden')
            insert_block(x1 + MCW/3 + 30.5, y2 - 30 - MDFirstSpace - 65 - through, "Ceiling_Bracket_horizontal_hole2")
            insert_block(x1 + MCW*2/3 - 30.5, y2 - 30 - MDFirstSpace - 65 - through, "Ceiling_Bracket_horizontal_hole2")
            insert_block(x1 + MCW/2, y2 - 30 - MDFirstSpace - 65 - through, "Ceiling_Bracket_horizontal_hole4")
            #중간 가로 브라켓 중 (MDSecondSpace가 400이상일 때만 그림)
            if ceilingdivision >= 2 and MDSecondSpace >= 400:
                rectangle(doc, x1 + MCW/3 + 0.5, y2 - 30 - MDFirstSpace - 130 - MDCenterBK - 65 - through/2, x1 + MCW*2/3 - 0.5, y2 - 30 - MDFirstSpace - 130 - MDCenterBK + 65 - through/2, layer='2')
                rectangle(doc, x1 + MCW/3 + 0.5, y2 - 30 - MDFirstSpace - 130 + 18 - MDCenterBK - 65 - through/2, x1 + MCW*2/3 - 0.5, y2 - 30 - MDFirstSpace - 130 + 112 - MDCenterBK - 65 - through/2, layer='2')
                rectangle(doc, x1 + MCW/3 + 0.5, y2 - 30 - MDFirstSpace - 130 + 16 - MDCenterBK - 65 - through/2, x1 + MCW*2/3 - 0.5, y2 - 30 - MDFirstSpace - 130 + 114 - MDCenterBK - 65 - through/2, layer='Hidden')
                insert_block(x1 + MCW/3 + 30.5, y2 - 30 - MDFirstSpace - 65 - MDCenterBK - 65 - through/2, "Ceiling_Bracket_horizontal_hole2")
                insert_block(x1 + MCW*2/3 - 30.5, y2 - 30 - MDFirstSpace - 65 - MDCenterBK - 65 - through/2, "Ceiling_Bracket_horizontal_hole2")
                insert_block(x1 + MCW/2, y2 - 30 - MDFirstSpace - 65 - MDCenterBK - 65 - through/2, "Ceiling_Bracket_horizontal_hole4")
            #중간 가로 브라켓 하
            rectangle(doc, x1 + MCW/3 + 0.5, y1 + ELYpos + 65 + MDFirstSpace, x1 + MCW*2/3 - 0.5, y1 + ELYpos + 65 + MDFirstSpace + 130, layer='2')
            rectangle(doc, x1 + MCW/3 + 0.5, y1 + ELYpos + 65 + MDFirstSpace + 18, x1 + MCW*2/3 - 0.5, y1 + ELYpos + 65 + MDFirstSpace + 130 - 18, layer='2')
            rectangle(doc, x1 + MCW/3 + 0.5, y1 + ELYpos + 65 + MDFirstSpace + 16, x1 + MCW*2/3 - 0.5, y1 + ELYpos + 65 + MDFirstSpace + 130 - 16, layer='Hidden')
            insert_block(x1 + MCW/3 + 30.5, y1 + ELYpos + 65 + MDFirstSpace + 65, "Ceiling_Bracket_horizontal_hole2")
            insert_block(x1 + MCW*2/3 - 30.5, y1 + ELYpos + 65 + MDFirstSpace + 65, "Ceiling_Bracket_horizontal_hole2")
            insert_block(x1 + MCW/2, y1 + ELYpos + 65 + MDFirstSpace + 65, "Ceiling_Bracket_horizontal_hole4")
            ...
            #중간 가로 브라켓 치수선
            if ceilingdivision == 1:
                dim(doc, x1 + MCW/3, y1 + ELYpos + 65.5, x1 + MCW/3, y2 - 30 - through, 100, direction="right", localdimstyle="0.2 JKW")
                dim(doc, x1 + MCW/3, y1 + ELYpos + 65.5, x1 + MCW/3, y1 + ELYpos + 65.5 + MDFirstSpace, 50, direction="right", localdimstyle="0.2 JKW")
                dimcontinue(doc, x1 + MCW/3, y1 + ELYpos + 65.5 + MDFirstSpace + 130)
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - MDFirstSpace - 130 - through)
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - MDFirstSpace - through)
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - through)
            else:
                dim(doc, x1 + MCW/3, y1 + ELYpos + 65.5, x1 + MCW/3, y2 - 30 - through, 100, direction="right", localdimstyle="0.2 JKW")
                ...
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - MDFirstSpace - 130 - MDCenterBK - 65 - through/2)
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - MDFirstSpace - 130 - MDCenterBK - 65 + 130 - through/2)
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - MDFirstSpace - 130 - through)
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - MDFirstSpace - through)
                dimcontinue(doc, x1 + MCW/3, y2 - 30 - through)
        ...
        #좌우 세로 긴 브라켓 길이 저장
        mcsizex3 = y2 - 30.5 - (y1 + ELYpos + 66) - through
        ...
        if ceilingdivision == 1:   
            #내부 가로 보강 치수선 좌
            dim(doc, x1 + 200, y1 + ELYpos + 65.5, x1 + 200, y2 - 700 - 15, 30, direction="right", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 200, y2 - 700 + 15)
            dimcontinue(doc, x1 + 200, y2 - 30)
            #내부 가로 보강 치수선 우
            dim(doc, x2 - 200, y1 + ELYpos + 65.5, x2 - 200, y2 - 700 - 15, 30, direction="right", localdimstyle="0.2 JKW")
            dimcontinue(doc, x2 - 200, y2 - 700 + 15)
            dimcontinue(doc, x2 - 200, y2 - 30)
        else:
            #내부 가로 보강 치수선 좌
            dim(doc, x1 + 200, y1 + ELYpos + 65.5, x1 + 200, y1 + (MCD - SplitYpos1) - 15, 30, direction="right", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 200, y1 + (MCD - SplitYpos1) + 15)
            dimcontinue(doc, x1 + 200, y2 - 700 - 15)
            dimcontinue(doc, x1 + 200, y2 - 700 + 15)
            dimcontinue(doc, x1 + 200, y2 - 30)
            #내부 가로 보강 치수선 우
            dim(doc, x2 - 200, y1 + ELYpos + 65.5, x2 - 200, y1 + (MCD - SplitYpos1) - 15, 30, direction="right", localdimstyle="0.2 JKW")
            dimcontinue(doc, x2 - 200, y1 + (MCD - SplitYpos1) + 15)
            dimcontinue(doc, x2 - 200, y2 - 700 - 15)
            dimcontinue(doc, x2 - 200, y2 - 700 + 15)
            dimcontinue(doc, x2 - 200, y2 - 30)
        ...
        #LH타입 팬, 공기청정기
        if LHtype == "LH":
            insert_block(x2 - 41.5, y2 - 157.1, "Ceiling_LH_Fan")#팬
            dim(doc, x2 - 41.5, y2 - 505.6, x2, y2 - 500, 100, direction="down", option="reverse", localdimstyle="0.2 JKW")
            dimcontinue(doc, x2 - 41.5 - 167.5, y2 - 505.6)
            insert_block(x1 + 53, y2 - 125, "Ceiling_LH_Aircleaner")#공기청정기
            dim(doc, x1 + 93.1, y2 - 625.1, x1, y2 - 625.1, -13, direction="down", option="reverse", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 93.1 + 190, y2 - 625.1)
            dim(doc, x1 + 53, y2 - 125, x1, y2, 50, direction="left", option="reverse", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 53, y2 - 125 - 360, 103)
            dim(doc, x1 + 93.1, y2 - 565.1, x1 + 93.1, y2 - 565.1 - 60, 100, direction="left", option="reverse", localdimstyle="0.2 JKW")
        else:
            #팬
            if entrytype == "관통":
                through_fan = 60
                ...
                through_fan = 0
            insert_block(x1 + 75, y2 - 140 - through_fan, "Ceiling_Fan")
            line(doc, x1 - 90, y1 + 25 - 140, x1 + 10, y1 + 25 - 140, layer="0")#140 선
            dim(doc, x1, y2, x1 + 70, y2 - 140 - through_fan, 100, direction="left", localdimstyle="0.2 JKW")#팬
            dimcontinue(doc, x1 + 70, y2 - 520 - through_fan)
            dim(doc, x1 + 75, y2 - 530 - through_fan, x1, y2 - 500 - through_fan, 100, direction="down", option="reverse", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 165, y2 - 530 - through_fan)
            if fan_su == 2:
                insert_block(x2 - 75, y2 - 140 - through_fan, "Ceiling_Fan_right")
            
            
        #서스테이너
        if secondord == "명원엘리베이터":
            insert_block(x1 + MCW/2 - AS_raw/2, y1 + 25 - 140 + EE_raw, "Ceiling_Sustainer_myungwon")
            insert_block(x1 + MCW/2 - AS_raw/2 + AS_raw, y1 + 25 - 140 + EE_raw, "Ceiling_Sustainer_myungwon_R")
        else:
        ...
        #하부 치수선
        textstr =  f"AS = {AS_raw}"
        dim(doc, x1 + MCW/2 - AS_raw/2, y1 + EE_raw - 200, x1 + MCW/2 - AS_raw/2 + AS_raw, y1 + EE_raw - 200, EE_raw + 50, direction="down", localdimstyle="0.2 JKW", text=textstr)#EE
        #우측 치수선
    ...
    if program:
        ...
        if entrytype == "관통":
            if headertype == "이노에스템":
                insert_block(x1, y2, "Ceiling_Bottom_section_inoestem_through")
            elif headertype == "정일산업":
                insert_block(x1, y2, "Ceiling_Bottom_section_joungil_through")
            elif headertype == "에이스코리아":
                insert_block(x1, y2, "Ceiling_Bottom_section_acekorea_through")
            elif headertype == "세화정공":
                insert_block(x1, y2, "Ceiling_Bottom_section_sehwa_through")
    ...
    if program:    
        ...
        if headertype == "세화정공":
            ...
            for i in range(1, lastNum + 1):
                ...
                line(doc, prev_x, prev_y, curr_x, curr_y, layer="레이져")
                prev_x, prev_y = curr_x, curr_y
            line(doc, prev_x, prev_y, temp_x1, temp_y1, layer="레이져")  
        ...
        if lctype == "035":
            ...
            for i, x in enumerate(Bottomx):
                ...
                if i == 0:
                    dim(doc, x1 + 25 + x, y1 + 55, x1, y1, 240, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                else:
                    dimcontinue(doc, x1 + x + 25, y1 + 50)
            dimcontinue(doc, x2, y1) 
            ...
            circle_cross(doc, x1 + 125, y1 + MCD/2, 13, layer='레이져')
            circle_cross(doc, x2 - 125, y1 + MCD/2, 13, layer='레이져')
            dim(doc, x1 + 125, y1 + MCD/2, x1, y1 + MCD/2, 50, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            dim(doc, x2 - 125, y1 + MCD/2, x2, y1 + MCD/2, 50, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            ...
            circle_cross(doc, x1 + 155, y1 + 65, 13, layer='레이져')
            circle_cross(doc, x1 + 155 + 160, y1 + 65, 13, layer='레이져')
            circle_cross(doc, x2 - 155, y1 + 65, 13, layer='레이져')
            circle_cross(doc, x2 - 155 - 160, y1 + 65, 13, layer='레이져')
            
            circle_cross(doc, x1 + 155, y2 - 65, 13, layer='레이져')
            circle_cross(doc, x1 + 155 + 160, y2 - 65, 13, layer='레이져')
            circle_cross(doc, x2 - 155, y2 - 65, 13, layer='레이져')
            circle_cross(doc, x2 - 155 - 160, y2 - 65, 13, layer='레이져')
            
            dim(doc, x1 + 155, y2 - 60, x1, y2, 180, direction="up", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 155 + 160, y2 - 60)
            dimcontinue(doc, x2 - (155 + 160), y2 - 60)
            dimcontinue(doc, x2 - 155, y2 - 60)
            dimcontinue(doc, x2, y2)
            ...
            circle_cross(doc, x1 + 162, y1 + 65, 13, layer='레이져')
            circle_cross(doc, x1 + 162 + 130, y1 + 65, 13, layer='레이져')
            circle_cross(doc, x2 - 162, y1 + 65, 13, layer='레이져')
            circle_cross(doc, x2 - 162 - 130, y1 + 65, 13, layer='레이져')
            
            circle_cross(doc, x1 + 162, y2 - 65, 13, layer='레이져')
            circle_cross(doc, x1 + 162 + 130, y2 - 65, 13, layer='레이져')
            circle_cross(doc, x2 - 162, y2 - 65, 13, layer='레이져')
            circle_cross(doc, x2 - 162 - 130, y2 - 65, 13, layer='레이져')
            
            dim(doc, x1 + 162, y2 - 60, x1, y2, 180, direction="up", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1 + 162 + 130, y2 - 60)
            dimcontinue(doc, x2 - (162 + 130), y2 - 60)
            dimcontinue(doc, x2 - 162, y2 - 60)
            dimcontinue(doc, x2, y2)
        ...
        if emergencytype == "있음":
            ...
            rectangle(doc, EXITx1, EXITy1, EXITx2, EXITy2, layer='레이져')
            line(doc, EXITx1, EXITy1, EXITx2, EXITy2, layer='0중심')
            line(doc, EXITx1, EXITy2, EXITx2, EXITy1, layer='0중심')
        ...
        dim_leader(doc, x1 + 80, y1 + slotposition, x1 + 80 + 50, y1 + slotposition + 100, text, direction="leftToright", distance=12)
            ...
            #LH타입 팬, 공기청정기
        if LHtype == "LH":
            insert_block(x1 + 53, y2 - 125, "Ceiling_LH_Aircleaner_laser")
            insert_block(x1 + 53, y2 - 125, "Ceiling_LH_Aircleaner_dim")
            insert_block(x2 - 41.5, y2 - 157, "Ceiling_LH_Fan_laser")
            insert_block(x2 - 41.5, y2 - 157, "Ceiling_LH_Fan_dim")
        else:
            if entrytype == "관통":
                through_fan = 60
            else:
                through_fan = 0
            insert_block(x1 + 75, y2 - 140 - through_fan, "Ceiling_Fan_laser")
            dim(doc, x1, y2, x1 + 43, y2 - 147 - through_fan, 50, direction="left", localdimstyle="0.2 JKW")#팬
            dimcontinue(doc, x1 + 43, y2 - 513 - through_fan)
            dim(doc, x1, y2, x1 + 70, y2 - 140 - through_fan, 100, direction="left", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 70, y2 - 520 - through_fan)
            dim(doc, x1 + 75, y2 - 530 - through_fan, x1, y2 - 500 - through_fan, 150 - through_fan, direction="down", option="reverse", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 165, y2 - 530 - through_fan)
            dim(doc, x1, y2 - 147 - through_fan, x1 + 43, y2 - 147 - through_fan, 100, direction="down", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 + 93, y2 - 147 - through_fan)
            text = "4-%%C9.2"
            dim_leader(doc, x1 + 165, y2 - 520 - through_fan, x1 + 165 + 50, y2 - 520 + 50 - through_fan, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       
            if fan_su == 2:
                insert_block(x2 - 75, y2 - 140 - through_fan, "Ceiling_Fan_laser_right")
                dim(doc, x2, y2, x2 - 43, y2 - 147 - through_fan, 50, direction="right", localdimstyle="0.2 JKW")#팬
                dimcontinue(doc, x2 - 43, y2 - 513 - through_fan, 93)
                dim(doc, x2, y2, x2 - 70, y2 - 140 - through_fan, 100, direction="right", localdimstyle="0.2 JKW")
                dimcontinue(doc, x2 - 70, y2 - 520 - through_fan, 170)
                dim(doc, x2 - 75, y2 - 530 - through_fan, x2, y2 - 500 - through_fan, 150 - through_fan, direction="down", option="reverse", localdimstyle="0.2 JKW")
                dimcontinue(doc, x2 - 165, y2 - 530 - through_fan)
                dim(doc, x2, y2 - 147 - through_fan, x2 - 43, y2 - 147 - through_fan, 100, direction="down", localdimstyle="0.2 JKW")
                dimcontinue(doc, x2 - 93, y2 - 147 - through_fan)
                text = "4-%%C9.2"
                dim_leader(doc, x2 - 165, y2 - 520 - through_fan, x2 - 165 - 50, y2 - 520 + 50 - through_fan, text, direction="rightToleft", distance=13)   # distance 13은 글자높이임       
        ...
        dim(doc, x1, y1, x2, y1, 260, direction="down", localdimstyle="0.2 JKW")
    ...
    if program:
        ...
        BasicXscale = 2813
        BasicYscale = 1765
        TargetXscale = BasicXscale + (max(CW, CD) - 1300) 
        TargetYscale = BasicYscale
        if(TargetXscale/BasicXscale > TargetYscale/BasicYscale):
            frame_scale = TargetXscale/BasicXscale
        else:
    ...
    ###################################################
        ...
        if headertype == "세화정공":
            ...
            #절곡라인  (실선)      
            line(doc, x7, y7, x14, y14, layer='2')     

            # 상부 전체치수
            dim(doc, x8, y8, x13, y13, 190, direction="up", option='reverse', localdimstyle="0.2 JKW")
            dim(doc, x10, y10, x8, y8, 140, direction="up", option='reverse', localdimstyle="0.2 JKW")
            dimcontinue(doc, x11, y11)
            dimcontinue(doc, x13, y13)
            # 좌측 치수선
            dim(doc, x8, y8, x10, y10, 50, direction="left", localdimstyle="0.2 JKW")
            # 우측 치수선
            dim(doc, x14, y14, x1, y1, 150, direction="right", option='reverse', localdimstyle="0.2 JKW")
            dimcontinue(doc, x11, y11)
            dim(doc, x1, y1, x11, y11, 200, direction="right", localdimstyle="0.2 JKW")
            
            text = "4-%%C30 Hole"
            dim_leader(doc, x6 + 200, y1 + 28, x6 + 200 + 50, y1 + 28 + 50, text, direction="leftToright", distance=17)       
            text = "%%C8x20 Slot"
            dim_leader(doc, x1 - 300, y1 + slotposition, x1 - 300 + 100, y1 + slotposition + 100, text, direction="leftToright", distance=12)
            text = "4-R20"
            dim_leader(doc, x11 - 10, y11 - 10, x11 + 50, y11 + 50, text, direction="leftToright", distance=17)
            
            dim(doc, x5, y5, x4, y4, 50, direction="right", localdimstyle="0.2 JKW")
            dim(doc, x5, y5, x2, y2, 50, direction="down", localdimstyle="0.2 JKW")
            dimcontinue(doc, x1, y1)
            
            circle_cross(doc, x6 + 200, y1 + 30 - thickness, 35, layer='레이져')
            circle_cross(doc, x6 + 200 + MCW/4, y1 + 30 - thickness, 35, layer='레이져')
            circle_cross(doc, x6 + 200 + MCW/2, y1 + 30 - thickness, 35, layer='레이져')
            circle_cross(doc, x1 - 200, y1 + 30 - thickness, 35, layer='레이져')
            
            #front 장공
            offset = 0
            for x in frontPanel[:-1] :
                offset = offset + x
                insert_block(x6 + offset, y1 + slotposition, "8x20 slot_Laser")
                dim(doc, x6 + offset, y1, x6 + offset - x, y1, 130, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x1, y1)
            dim(doc, x6, y6, x6 + 50, y6 + slotposition, 50, direction="left", localdimstyle="0.2 JKW")    
            ...
            if 700 <= OP_raw <= 1100:
                ...
                block_name = f"Ceiling_foot_OP1100"
                block_name_laser = f"Ceiling_foot_OP1100_laser"
                
            insert_block(x5, y5, block_name)
            insert_block(x5, y5, block_name_laser)
                        
            #절곡라인  (실선)      
            line(doc, x8, y8, x3, y3, layer='2')     
            ...
            for x in frontPanel[:-1] :
                offset = offset + x
                insert_block(x1 + offset, y1 + slotposition, "8x20 slot_Laser")
                dim(doc, x1 + offset, y1, x1 + offset - x, y1, 70, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            dimcontinue(doc, x2, y2)
            dim(doc, x1, y1, x1 + 50, y1 + slotposition, 50, direction="left", localdimstyle="0.2 JKW")    

        # main Frame description    
        if headertype == "세화정공":
            x1 = x6 + max(MCW, MCD) + 200
        else:
    ...
    ###################################################
        ...
        if secondord == "명원엘리베이터":
            ...
            x4 = x3
            y4 = y3 + 130 - thickness
            x5 = x4 - MCD + (25 - 140 + EE_raw + 185) + 31
            y5 = y4
            x6 = x5
            y6 = y5 - 100
            x7 = x6 - 370
        ...
        #절곡라인  (실선)      
        line(doc, x10, y10, x3, y3, layer='2')     
        #side장공
        offset = 0
        for i, x in enumerate(sidePanel[:-1]) :
            offset = offset + x
            insert_block(x1 + offset - 31, y1 + slotposition, "8x20 slot_Laser")
            if i== 0 :
                fixX1 = x1 + offset - 31
                fixX2 = x1 + offset - x
                print(f"첫번째 x : {fixX1}, 두번째 x : {fixX2}")
                dim(doc, fixX1 , y1, fixX2 , y1, 70, direction="down", option='reverse', localdimstyle="0.2 JKW")    
            else:
                dim(doc, x1 + offset - 31, y1, x1 + offset - x - 31, y1, 70, direction="down", option='reverse', localdimstyle="0.2 JKW")    
        dimcontinue(doc, x2, y2)
        dim(doc, x1, y1, x1 + 50, y1 + slotposition, 50, direction="left", localdimstyle="0.2 JKW")    
    ...
    ###################################################
        ...
        if headertype == "세화정공":
            ...
            if lctype == "035":
                lc_positions = calculate_lc_position_035(inputCW, inputCD)
                for i, x in enumerate(Bottomx):
                    insert_block(x1 - 6 + x, y1 + 70, "Ceiling_LChole_BK_sehwa")
                    if i == 0:
                ...
                for i, x in enumerate(Bottomx):
                    if i != 0:
                        ...
                        line(doc, x24 - 6 + Bottomx[i-1] + 17.5, y24, x24 - 6 + x - 17.5, y24, layer="레이져")
                line(doc, x24, y24, x24 + 86.5, y24, layer="레이져")
                line(doc, x15, y15, x15 - 86.5, y24, layer="레이져")    
                ...
                for i, x in enumerate(Bottomx):
                    if i != 0:
                        # 나머지 선: (x + 17.5, y12) -> (다음 x - 17.5, y12)
                        line(doc, x24 - 6 + Bottomx[i-1] + 17.5, y24, x24 - 6 + x - 17.5, y24, layer="레이져")
                line(doc, x24, y24, x24 + 91.5, y24, layer="레이져")
                line(doc, x15, y15, x15 - 91.5, y24, layer="레이져")    
                
            elif lctype == "008_A":
                lc_positions = calculate_lc_position_008_A(inputCW, inputCD)
                for i, x in enumerate(Bottomx):
                    insert_block(x1 - 6 + x, y1 + 65, "Ceiling_LChole_BK_sehwa_008A")
                    if i == 0:
                        dim(doc, x1 - 6 + x, y1 + 50, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    else:
                        dimcontinue(doc, x1 - 6 + x, y1 + 50)
                dimcontinue(doc, x10, y1) 
                ...
                for i, x in enumerate(Bottomx):
                    if i != 0:
                        # 나머지 선: (x + 17.5, y12) -> (다음 x - 17.5, y12)
                        line(doc, x24 - 6 + Bottomx[i-1] + 17.5, y24, x24 - 6 + x - 17.5, y24, layer="레이져")
                line(doc, x24, y24, x24 + 51.5, y24, layer="레이져")
                line(doc, x15, y15, x15 - 51.5, y24, layer="레이져")    
                
            elif lctype == "N20":
                insert_block(x1 + 106.5, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x1 + 106.5 + 185, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 106.5, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 106.5 - 185, y1 + 70, "Ceiling_LChole_BK_sehwa") 
                
                line(doc, x15, y15, x15 - 89, y15, layer="레이져")
                line(doc, x15 - 124, y15, x15 - 274, y15, layer="레이져")
                line(doc, x15 - 309, y15, x15 - (CW/2-131), y15, layer="레이져")
                line(doc, x15 - (CW/2-81), y15, x15 - (CW/2-81) - 200, y15, layer="레이져")
                line(doc, x24, y24, x24 + 89, y24, layer="레이져")
                line(doc, x24 + 124, y24, x24 + 274, y24, layer="레이져")
                line(doc, x24 + 309, y24, x24 + (CW/2-181), y24, layer="레이져")
                
                dim(doc, x1 + 106.5, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                dimcontinue(doc, x1 + 106.5 + 185, y1 + 52.5)
                dimcontinue(doc, x10 - 106.5 - 185, y1 + 52.5)
                dimcontinue(doc, x10 - 106.5, y1 + 52.5)
                dimcontinue(doc, x10, y10)
                
            elif lctype == "032":
                insert_block(x1 + 139, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x1 + 139 + 150, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 139, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 139 - 150, y1 + 70, "Ceiling_LChole_BK_sehwa") 
                
                line(doc, x15, y15, x15 - 121.5, y15, layer="레이져")
                line(doc, x15 - 156.5, y15, x15 - 271.5, y15, layer="레이져")
                line(doc, x15 - 306.5, y15, x15 - (CW/2-131), y15, layer="레이져")
                line(doc, x15 - (CW/2-81), y15, x15 - (CW/2-81) - 200, y15, layer="레이져")
                line(doc, x24, y24, x24 + 121.5, y24, layer="레이져")
                line(doc, x24 + 156.5, y24, x24 + 271.5, y24, layer="레이져")
                line(doc, x24 + 306.5, y24, x24 + (CW/2-181), y24, layer="레이져")
                
                dim(doc, x1 + 139, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                dimcontinue(doc, x1 + 139 + 150, y1 + 52.5)
                dimcontinue(doc, x10 - 139 - 150, y1 + 52.5)
                dimcontinue(doc, x10 - 139, y1 + 52.5)
                dimcontinue(doc, x10, y10)
                
            elif lctype in ("026", "012"):
                insert_block(x1 + 134, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x1 + 134 + 160, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 134, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 134 - 160, y1 + 70, "Ceiling_LChole_BK_sehwa") 
                
                line(doc, x15, y15, x15 - 116.5, y15, layer="레이져")
                line(doc, x15 - 151.5, y15, x15 - 276.5, y15, layer="레이져")
                line(doc, x15 - 311.5, y15, x15 - (CW/2-131), y15, layer="레이져")
                line(doc, x15 - (CW/2-81), y15, x15 - (CW/2-81) - 200, y15, layer="레이져")
                line(doc, x24, y24, x24 + 116.5, y24, layer="레이져")
                line(doc, x24 + 151.5, y24, x24 + 276.5, y24, layer="레이져")
                line(doc, x24 + 311.5, y24, x24 + (CW/2-181), y24, layer="레이져")
                
                dim(doc, x1 + 134, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                dimcontinue(doc, x1 + 134 + 160, y1 + 52.5)
                dimcontinue(doc, x10 - 134 - 160, y1 + 52.5)
                dimcontinue(doc, x10 - 134, y1 + 52.5)
                dimcontinue(doc, x10, y10)
                
            elif lctype in ("011", "017", "025"):
                insert_block(x1 + 124, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x1 + 124 + 160, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 124, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 124 - 160, y1 + 70, "Ceiling_LChole_BK_sehwa") 
                
                line(doc, x15, y15, x15 - 106.5, y15, layer="레이져")
                line(doc, x15 - 141.5, y15, x15 - 266.5, y15, layer="레이져")
                line(doc, x15 - 301.5, y15, x15 - (CW/2-131), y15, layer="레이져")
                line(doc, x15 - (CW/2-81), y15, x15 - (CW/2-81) - 200, y15, layer="레이져")
                line(doc, x24, y24, x24 + 106.5, y24, layer="레이져")
                line(doc, x24 + 141.5, y24, x24 + 266.5, y24, layer="레이져")
                line(doc, x24 + 301.5, y24, x24 + (CW/2-181), y24, layer="레이져")
                
                dim(doc, x1 + 124, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                dimcontinue(doc, x1 + 124 + 160, y1 + 52.5)
                dimcontinue(doc, x10 - 124 - 160, y1 + 52.5)
                dimcontinue(doc, x10 - 124, y1 + 52.5)
                dimcontinue(doc, x10, y10)
            
            elif lctype in ("013", "013D"):
                insert_block(x1 + 131, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x1 + 131 + 130, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 131, y1 + 70, "Ceiling_LChole_BK_sehwa")
                insert_block(x10 - 131 - 130, y1 + 70, "Ceiling_LChole_BK_sehwa") 
                
                line(doc, x15, y15, x15 - 113.5, y15, layer="레이져")
                line(doc, x15 - 148.5, y15, x15 - 243.5, y15, layer="레이져")
                line(doc, x15 - 278.5, y15, x15 - (CW/2-131), y15, layer="레이져")
                line(doc, x15 - (CW/2-81), y15, x15 - (CW/2-81) - 200, y15, layer="레이져")
                line(doc, x24, y24, x24 + 113.5, y24, layer="레이져")
                line(doc, x24 + 148.5, y24, x24 + 243.5, y24, layer="레이져")
                line(doc, x24 + 278.5, y24, x24 + (CW/2-181), y24, layer="레이져")
                
                dim(doc, x1 + 131, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                dimcontinue(doc, x1 + 131 + 130, y1 + 52.5)
                dimcontinue(doc, x10 - 131 - 130, y1 + 52.5)
                dimcontinue(doc, x10 - 131, y1 + 52.5)
                dimcontinue(doc, x10, y10)
            ...
            if lctype == "008_A":
                ...
                for i in range(1, lastNum + 1):
                    ...
                    if i == 11:
                        ...
                        prev_x, prev_y = curr_x, curr_y
                line(doc, prev_x, prev_y, x1, y1, layer="레이져")
                line(doc, x11 - 38, y11, x9 - (CW-12)/2 + 19, y11, layer="레이져")
                line(doc, x12 + 38, y11, x14 + (CW-12)/2 - 19, y11, layer="레이져")
                ...
                insert_block(x13, y13, "Ceiling_LChole_BK_left_008_A")
                insert_block(x1 + (CW-12)/2, y11, "Ceiling_LChole_BK_008_A")
                insert_block(x10, y10, "Ceiling_LChole_BK_right_008_A")
                
                lc_positions = calculate_lc_position_008_A(inputCW, inputCD)
                for i, x in enumerate(Bottomx):
                    if i == 0:
                        dim(doc, x1 - 6 + x, y1 + 92, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    else:
                        dimcontinue(doc, x1 - 6 + x, y1 + 92)
                dimcontinue(doc, x6, y6) 
                ...
                if lctype == "035":
                    ...
                    dimcontinue(doc, x1, y1)
                    dimcontinue(doc, x3, y3, 160, option='reverse')
                    dim(doc, x3, y3, x3 + 30, y3 + 94.5, 100, direction="left", localdimstyle="0.2 JKW")
                    dimcontinue(doc, x12, y12)
            
                    lc_positions = calculate_lc_position_035(inputCW, inputCD)
                    for i, x in enumerate(Bottomx):
                        circle_cross(doc, x1 - 6 + x, y1 + 76.5, 35, layer='레이져')
                        if i == 0:
                            dim(doc, x1 - 6 + x, y1 + 53, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                        else:
                            dimcontinue(doc, x1 - 6 + x, y1 + 53)
                    dimcontinue(doc, x6, y6) 
                    
                    text = "4-%%C35"
                    dim_leader(doc, x1 + 104, y1 + 76.5, x1 + 104 + 50, y1 + 76.5 + 50, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       
                    ...
                    dimcontinue(doc, x1, y1)
                    dimcontinue(doc, x3, y3, 160, option='reverse')
                    dim(doc, x3, y3, x3 + 67, y3 + 94.5, 100, direction="left", localdimstyle="0.2 JKW")
                    dimcontinue(doc, x12, y12)
                    # 하부 치수
                    dim(doc, x1 + 106.5, y1 + 54, x1, y1, 160, direction="down", option='reverse', localdimstyle="0.2 JKW")
                    dimcontinue(doc, x1 + 106.5 + 185, y1 + 54)
                    dimcontinue(doc, x6 - 106.5 - 185, y1 + 54)
                    dimcontinue(doc, x6 - 106.5, y1 + 54)
                    dimcontinue(doc, x6, y1)
            
                    circle_cross(doc, x1 + 106.5, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x1 + 106.5 + 185, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 106.5, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 106.5 - 185, y3 + 94.5, 35, layer='레이져')
                    
                    text = "4-%%C35"
                    dim_leader(doc, x1 + 106.5, y1 + 76.5, x1 + 106.5 + 50, y1 + 76.5 + 50, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       
                    ...
                    dimcontinue(doc, x12, y12)
                    # 하부 치수
                    dim(doc, x1 + 139, y1 + 54, x1, y1, 160, direction="down", option='reverse', localdimstyle="0.2 JKW")
                    dimcontinue(doc, x1 + 139 + 150, y1 + 54)
                    dimcontinue(doc, x6 - 139 - 150, y1 + 54)
                    dimcontinue(doc, x6 - 139, y1 + 54)
                    dimcontinue(doc, x6, y1)
            
                    circle_cross(doc, x1 + 139, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x1 + 139 + 150, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 139, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 139 - 150, y3 + 94.5, 35, layer='레이져')
                    
                    text = "4-%%C35"
                    dim_leader(doc, x1 + 139, y1 + 76.5, x1 + 139 + 50, y1 + 76.5 + 50, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       
                    ...
                    dimcontinue(doc, x1, y1)
                    dimcontinue(doc, x3, y3, 160, option='reverse')
                    dim(doc, x3, y3, x3 + 62, y3 + 94.5, 100, direction="left", localdimstyle="0.2 JKW")
                    dimcontinue(doc, x12, y12)
                    # 하부 치수
                    dim(doc, x1 + 134, y1 + 54, x1, y1, 160, direction="down", option='reverse', localdimstyle="0.2 JKW")
                    dimcontinue(doc, x1 + 134 + 160, y1 + 54)
                    dimcontinue(doc, x6 - 134 - 160, y1 + 54)
                    dimcontinue(doc, x6 - 134, y1 + 54)
                    dimcontinue(doc, x6, y1)
            
                    circle_cross(doc, x1 + 134, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x1 + 134 + 160, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 134, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 134 - 160, y3 + 94.5, 35, layer='레이져')
                    
                    text = "4-%%C35"
                    dim_leader(doc, x1 + 134, y1 + 76.5, x1 + 134 + 50, y1 + 76.5 + 50, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       
                    ...
                    dimcontinue(doc, x12, y12)
                    # 하부 치수
                    dim(doc, x1 + 124, y1 + 54, x1, y1, 160, direction="down", option='reverse', localdimstyle="0.2 JKW")
                    dimcontinue(doc, x1 + 124 + 160, y1 + 54)
                    dimcontinue(doc, x6 - 124 - 160, y1 + 54)
                    dimcontinue(doc, x6 - 124, y1 + 54)
                    dimcontinue(doc, x6, y1)
            
                    circle_cross(doc, x1 + 124, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x1 + 124 + 160, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 124, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 124 - 160, y3 + 94.5, 35, layer='레이져')
                    
                    text = "4-%%C35"
                    dim_leader(doc, x1 + 124, y1 + 76.5, x1 + 124 + 50, y1 + 76.5 + 50, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       

                elif lctype in ("013", "013D"):
                    # 좌측 치수
                    dim(doc, x12, y12, x14, y14, 130, direction="left", localdimstyle="0.2 JKW")
                    dimcontinue(doc, x1, y1)
                    dimcontinue(doc, x3, y3, 130, option='reverse')
                    dim(doc, x3, y3, x3 + 62, y3 + 94.5, 100, direction="left", localdimstyle="0.2 JKW")
                    dimcontinue(doc, x12, y12)
                    # 하부 치수
                    dim(doc, x1 + 131, y1 + 54, x1, y1, 130, direction="down", option='reverse', localdimstyle="0.2 JKW")
                    dimcontinue(doc, x1 + 131 + 130, y1 + 54)
                    dimcontinue(doc, x6 - 131 - 130, y1 + 54)
                    dimcontinue(doc, x6 - 131, y1 + 54)
                    dimcontinue(doc, x6, y1)
            
                    circle_cross(doc, x1 + 131, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x1 + 131 + 130, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 131, y3 + 94.5, 35, layer='레이져')
                    circle_cross(doc, x6 - 131 - 130, y3 + 94.5, 35, layer='레이져')
                    
                    text = "4-%%C35"
                    dim_leader(doc, x1 + 131, y1 + 76.5, x1 + 131 + 50, y1 + 76.5 + 50, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       
        ...
        elif headertype in ["에이스코리아", "이노에스템"]:
    ...
    ###################################################
        ...
        if headertype in ["에이스코리아", "이노에스템"]:
            ...
            if lctype == "008_A":
                ...
                insert_block(x13, y13, "Ceiling_LChole_BK_left_008_A_ace")
                insert_block(x9 - (CW-12)/2, y11, "Ceiling_LChole_BK_center_008_A_ace")
                insert_block(x10, y10, "Ceiling_LChole_BK_right_008_A_ace")
                ...
                line(doc, x12 + 38, y12, x9 - (CW-12)/2 - 19, y12, layer="레이져")
                line(doc, x11 - 38, y12, x14 + (CW-12)/2 + 19, y12, layer="레이져")
                ...
                if lctype == "035":
                    ...
                    lc_positions = calculate_lc_position_031(inputCW, inputCD)
                    for i, x in enumerate(Bottomx):
                        insert_block(x1 - 6 + x, y1 + 56, "Ceiling_LChole_BK_sehwa")
                        if i == 0:
                            dim(doc, x1 - 6 + x, y1 + 30, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                        else:
                            dimcontinue(doc, x1 - 6 + x, y1 + 50)
                    dimcontinue(doc, x6, y6) 
                    ...
                    for i, x in enumerate(Bottomx):
                        if i != 0:
                            # 나머지 선: (x + 17.5, y12) -> (다음 x - 17.5, y12)
                            line(doc, x12 - 56 + Bottomx[i-1] + 17.5, y12, x12 - 56 + x - 17.5, y12, layer="레이져")
                    line(doc, x12, y12, x12 + 41.5, y12, layer="레이져")
                    line(doc, x11, y11, x11 - 41.5, y12, layer="레이져")
                    
                elif lctype == "N20":
                    insert_block(x1 + 106.5, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x1 + 106.5 + 185, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 106.5, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 106.5 - 185, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    
                    line(doc, x11, y11, x11 - 39, y11, layer="레이져")
                    line(doc, x11 - 74, y11, x11 - 224, y11, layer="레이져")
                    line(doc, x11 - 259, y11, x12 + 259, y12, layer="레이져")
                    line(doc, x12 + 74, y12, x12 + 224, y11, layer="레이져")
                    line(doc, x12, y12, x12 + 39, y12, layer="레이져")
                    
                    dim(doc, x1 + 106.5, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    dimcontinue(doc, x1 + 106.5 + 185, y1 + 52.5)
                    dimcontinue(doc, x6 - 106.5 - 185, y1 + 52.5)
                    dimcontinue(doc, x6 - 106.5, y1 + 52.5)
                    dimcontinue(doc, x6, y6)
                    
                elif lctype == "032":
                    insert_block(x1 + 139, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x1 + 139 + 150, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 139, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 139 - 150, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    
                    line(doc, x11, y11, x11 - 71.5, y11, layer="레이져")
                    line(doc, x11 - 106.5, y11, x11 - 221.5, y11, layer="레이져")
                    line(doc, x11 - 256.5, y11, x12 + 256.5, y12, layer="레이져")
                    line(doc, x12 + 106.5, y12, x12 + 221.5, y11, layer="레이져")
                    line(doc, x12, y12, x12 + 71.5, y12, layer="레이져")
                    
                    dim(doc, x1 + 139, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    dimcontinue(doc, x1 + 139 + 150, y1 + 52.5)
                    dimcontinue(doc, x6 - 139 - 150, y1 + 52.5)
                    dimcontinue(doc, x6 - 139, y1 + 52.5)
                    dimcontinue(doc, x6, y6)
                    
                elif lctype in ("026", "012"):
                    insert_block(x1 + 134, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x1 + 134 + 160, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 134, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 134 - 160, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    
                    line(doc, x11, y11, x11 - 66.5, y11, layer="레이져")
                    line(doc, x11 - 101.5, y11, x11 - 226.5, y11, layer="레이져")
                    line(doc, x11 - 261.5, y11, x12 + 261.5, y12, layer="레이져")
                    line(doc, x12 + 101.5, y12, x12 + 226.5, y11, layer="레이져")
                    line(doc, x12, y12, x12 + 66.5, y12, layer="레이져")
                    
                    dim(doc, x1 + 134, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    dimcontinue(doc, x1 + 134 + 160, y1 + 52.5)
                    dimcontinue(doc, x6 - 134 - 160, y1 + 52.5)
                    dimcontinue(doc, x6 - 134, y1 + 52.5)
                    dimcontinue(doc, x6, y6)
                    
                elif lctype in ("011", "017", "025"):
                    insert_block(x1 + 124, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x1 + 124 + 160, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 124, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 124 - 160, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    
                    line(doc, x11, y11, x11 - 56.5, y11, layer="레이져")
                    line(doc, x11 - 91.5, y11, x11 - 216.5, y11, layer="레이져")
                    line(doc, x11 - 251.5, y11, x12 + 251.5, y12, layer="레이져")
                    line(doc, x12 + 91.5, y12, x12 + 216.5, y11, layer="레이져")
                    line(doc, x12, y12, x12 + 56.5, y12, layer="레이져")
                    
                    dim(doc, x1 + 124, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    dimcontinue(doc, x1 + 124 + 160, y1 + 52.5)
                    dimcontinue(doc, x6 - 124 - 160, y1 + 52.5)
                    dimcontinue(doc, x6 - 124, y1 + 52.5)
                    dimcontinue(doc, x6, y6)
                
                elif lctype in ("013", "013D"):
                    insert_block(x1 + 131, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x1 + 131 + 130, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 131, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    insert_block(x7 - 131 - 130, y1 + 56, "Ceiling_LChole_BK_sehwa")
                    
                    line(doc, x11, y11, x11 - 63.5, y11, layer="레이져")
                    line(doc, x11 - 98.5, y11, x11 - 193.5, y11, layer="레이져")
                    line(doc, x11 - 228.5, y11, x12 + 228.5, y12, layer="레이져")
                    line(doc, x12 + 98.5, y12, x12 + 193.5, y11, layer="레이져")
                    line(doc, x12, y12, x12 + 63.5, y12, layer="레이져")
                    
                    dim(doc, x1 + 131, y1 + 52.5, x1, y1, 190, direction="down", option='reverse', localdimstyle="0.2 JKW")    
                    dimcontinue(doc, x1 + 131 + 130, y1 + 52.5)
                    dimcontinue(doc, x6 - 131 - 130, y1 + 52.5)
                    dimcontinue(doc, x6 - 131, y1 + 52.5)
                    dimcontinue(doc, x6, y6)
    ...
    ###################################################
        ...
        if emergencytype == "있음":
            ceiling_bksize = EXsizex1
        else:
            ceiling_bksize = mcsizex2
            
        x1 = rx1 + 150
        y1 = ry1
        x2 = x1 + ceiling_bksize
        y2 = y1
        x3 = x2
        y3 = y2 + 28
        x4 = x3
        ...
        dim(doc, x1, y1, x1 + ceiling_bksize/2, y1, 60, direction="down", localdimstyle="0.2 JKW")
        dimcontinue(doc, x2, y2, 60)
        
        circle_cross(doc, x1 + ceiling_bksize/2, y3, 30, layer='레이져')
        text = "%%C30"
        dim_leader(doc, x1 + ceiling_bksize/2, y3, x1 + ceiling_bksize/2 + 50, y3 + 70, text, direction="leftToright", distance=13)   # distance 13은 글자높이임       
            
        # main Frame description    
        x1 = x1 + ceiling_bksize + 200
        y1 = y1 + 50
        textstr = f"Part Name : Ceiling Bracket"    
        draw_Text(doc, x1, y1, 30, str(textstr), layer='0')
        textstr = f"Mat.Spec : SPCC 2.3t"    
        draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
        textstr = f"Size : 56 x {ceiling_bksize:.1f}mm"
        draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
        if ceilingdivision == 1:
            textstr = f"Quantity : {su*2} EA"    
        else:
    ...
    ###################################################
        if secondord == "명원엘리베이터":
            rx1 = x1 + ceiling_bksize + 500
            ry1 = ry1 - 50
            insert_block(rx1, ry1, "Ceiling_Bracket_myungwon")    
            insert_block(rx1, ry1, "Ceiling_Bracket_myungwon_laser")    
            
            # main Frame description    
            x1 = x1 + ceiling_bksize + 1000
            textstr = f"Part Name : Panel Fixed Bracket Base"    
            draw_Text(doc, x1, y1, 30, str(textstr), layer='0')    
            textstr = f"Mat.Spec : SPCC 3.2t"    
            draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')    
            textstr = f"Size : 114 x 270mm"    
            draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')        
            textstr = f"Quantity : {su*2} EA"    
            draw_Text(doc, x1, y1 - 180, 30, str(textstr), layer='0')    
        else:
            rx1 = x1 + ceiling_bksize + 600
            ry1 = ry1 - 50
            insert_block(rx1, ry1, "Ceiling_Bracket_sustainer")    
            insert_block(rx1, ry1, "Ceiling_Bracket_sustainer_L")    
            
            # main Frame description    
            x1 = x1 + ceiling_bksize + 1200
            textstr = f"Part Name : Ceiling Bracket"    
            draw_Text(doc, x1, y1, 30, str(textstr), layer='0')    
            textstr = f"Mat.Spec : SPCC 2.3t"    
            draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')    
            textstr = f"Size : 96 x 300mm"    
            draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')        
            textstr = f"Quantity : {su*2} EA"    
            draw_Text(doc, x1, y1 - 180, 30, str(textstr), layer='0')    
    ...
    ###################################################
        ...
        if emergencytype == "있음":
            rectangle(doc, x1 - 31 + MCW/2 - emergencyRow/2 - 94 - 45, y1 + 26.5, x1 - 31 + MCW/2 - emergencyRow/2 - 94 + 45, y1 + 43.5, layer='레이져')
            rectangle(doc, x1 - 31 + MCW/2 - emergencyRow/2 - 94 - 45, y4 - 26.5, x1 - 31 + MCW/2 - emergencyRow/2 - 94 + 45, y4 - 43.5, layer='레이져')
            rectangle(doc, x2 + 31 - MCW/2 + emergencyRow/2 + 94 - 45, y1 + 26.5, x2 + 31 - MCW/2 + emergencyRow/2 + 94 + 45, y1 + 43.5, layer='레이져')
            rectangle(doc, x2 + 31 - MCW/2 + emergencyRow/2 + 94 - 45, y4 - 26.5, x2 + 31 - MCW/2 + emergencyRow/2 + 94 + 45, y4 - 43.5, layer='레이져')

            dim(doc, x1 - 31 + MCW/2 - emergencyRow/2 - 94 - 45, y1 + 26.5, x1, y1, 100, direction="down", option='reverse', localdimstyle="0.2 JKW")
            dimcontinue(doc, x1 - 31 + MCW/2 - emergencyRow/2 - 94 + 45, y1 + 26.5)
            if CW>1400:
                dimcontinue(doc, x1 + (CW - 12)/2 - 45, y1 + 26.5)
                dimcontinue(doc, x1 + (CW - 12)/2 + 45, y1 + 26.5)
                dimcontinue(doc, x2 + 31 - MCW/2 + emergencyRow/2 + 94 - 45, y1 + 26.5)
                dimcontinue(doc, x2 + 31 - MCW/2 + emergencyRow/2 + 94 + 45, y1 + 26.5)
            else:    
                dimcontinue(doc, x2 + 31 - MCW/2 + emergencyRow/2 + 94 - 45, y1 + 26.5)
                dimcontinue(doc, x2 + 31 - MCW/2 + emergencyRow/2 + 94 + 45, y1 + 26.5)
        ...
        #절곡라인  (히든선 : 정방향)      
        if CW>1400:
            rectangle(doc, x1 + (CW - 12)/2 - 45, y1 + 26.5, x1 + (CW - 12)/2 + 45, y1 + 43.5, layer='레이져')
            rectangle(doc, x1 + (CW - 12)/2 - 45, y4 - 26.5, x1 + (CW - 12)/2 + 45, y4 - 43.5, layer='레이져')
    ...
    ###################################################
        ...
        frameXpos = frameXpos + TargetXscale + 800
    ...
    ###################################################
        ...
        if emergencytype == "있음" and CW < 1700:
            x1 = rx1 + 300
            y1 = ry1
            x2 = x1 + mcsizex3
            y2 = y1
            ...
            insert_block(x1, y1, "Ceiling_Bracket_horizontal_L2_100")
            insert_block(x2, y2, "Ceiling_Bracket_horizontal_R_2_100")
            insert_block(x1 + (MCD - (ELYpos + 96.5) - through)/3, y1 + 75.6, "Ceiling_Bracket_horizontal_hole4_laser_100")
            insert_block(x1 + (MCD - (ELYpos + 96.5) - through)*2/3, y1 + 75.6, "Ceiling_Bracket_horizontal_hole4_laser_100")
            ...
            insert_block(x1 + (MCD - (ELYpos + 96.5) - through)/3, y1 + 90.6, "Ceiling_Bracket_horizontal_hole4_laser")
            insert_block(x1 + (MCD - (ELYpos + 96.5) - through)*2/3, y1 + 90.6, "Ceiling_Bracket_horizontal_hole4_laser")
        ...
        dimcontinue(doc, x4 + (MCD - (ELYpos + 96.5) - through)/3 - 30, y4)
        dimcontinue(doc, x4 + (MCD - (ELYpos + 96.5) - through)/3 + 30, y4)
        dimcontinue(doc, x4 + (MCD - (ELYpos + 96.5) - through)*2/3 - 30, y4)
        dimcontinue(doc, x4 + (MCD - (ELYpos + 96.5) - through)*2/3 + 30, y4)
        dimcontinue(doc, x3 - 30, y4)
        dimcontinue(doc, x3, y4)
        # 하부 전체치수
        ...
        temp_y1 = y1 - 200
        textstr = f"Part Name : Ceiling Bracket"
        draw_Text(doc, temp_x1, temp_y1, 30, str(textstr), layer='0')
        textstr = f"Mat.Spec : SPCC 1.6T"
        draw_Text(doc, temp_x1, temp_y1 - 60, 30, str(textstr), layer='0')
        textstr = f"Size : {ceilingbk130} x {mcsizex3:.1f}mm"
        draw_Text(doc, temp_x1, temp_y1 - 120, 30, str(textstr), layer='0')
        textstr = f"Quantity : {su*2} EA"
        draw_Text(doc, temp_x1, temp_y1 - 180, 30, str(textstr), layer='0')
    ...
    ###################################################
        ...
        if emergencytype == "있음" and CW < 1700:
            ...
            insert_block(rx1, ry1, "Ceiling_Coverplate_100")
            insert_block(rx1, ry1, "Ceiling_Coverplate_laser_100")
            insert_block(x2, y2, "Ceiling_CoverplateR_100")
            insert_block(x2, y2, "Ceiling_Coverplate_laserR_100")
            ...
            coverplatex2 = (MCD - (ELYpos + 96.5) - through)/3 - 62 + 58
        ...
        textstr = f"Mat.Spec : SPCC 1.6t"    
        draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
        textstr = f"Size : {ceilingcover130} x {coverplatex2:.1f}mm"    
        draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
    ...
    ###################################################
        ...
        if emergencytype == "있음": 
            ...
            text = "4-%%C3.2"
            dim_leader(doc, x1 + 30, y1 + 9, x1 + 30 + 50, y1 + 9 + 70, text, direction="leftToright", distance=13)
            ...
            insert_block(x1 + (x2 - x1)/2, y1 + 90.6, "Ceiling_Bracket_horizontal_hole4_laser")

            text = "8-%%C3.2"
            dim_leader(doc, x1 + 30, y1 + 9, x1 + 30 + 50, y1 + 9 + 70, text, direction="leftToright", distance=13)
        
            dim(doc, x4, y4, x4 + 30, y4, 100, direction="up", localdimstyle="0.2 JKW")
            dimcontinue(doc, x4 + (x2 - x1)/2 - 30, y4)
            dimcontinue(doc, x4 + (x2 - x1)/2 + 30, y4)
            dimcontinue(doc, x3 - 30, y4)
            dimcontinue(doc, x3, y4)
        ...
        textstr = f"Mat.Spec : SPCC 1.6T"    
        draw_Text(doc, temp_x1, temp_y1 - 60, 30, str(textstr), layer='0')    
        textstr = f"Size : 181.2 x {(x2 - x1):.1f}mm"    
        draw_Text(doc, temp_x1, temp_y1 - 120, 30, str(textstr), layer='0')        
        
        rectangle(doc, x1 + (x2 - x1)/2 - 45, y1 + 26.5, x1 + (x2 - x1)/2 + 45, y1 + 43.5, layer='레이져')
        rectangle(doc, x1 + (x2 - x1)/2 - 45, y4 - 26.5, x1 + (x2 - x1)/2 + 45, y4 - 43.5, layer='레이져')
        ...
        # 하부 전체치수
        dim(doc, x1 + (x2 - x1)/2 - 45, y1 + 26.5, x1, y1, 100, direction="down", option='reverse', localdimstyle="0.2 JKW")
        dimcontinue(doc, x1 + (x2 - x1)/2 + 45, y1 + 26.5)
        dimcontinue(doc, x2, y2)
    ...
    #################################################################################################
    # 8page Ceiling  Bracket (비상구출구 있을때만 작도)
    #################################################################################################
        if emergencytype == "있음": 
            ...
            ###################################################
                ...
                rectangle(doc, x1 + EXsizex3 - 45, y1 + 26.5, x1 + EXsizex3 + 45, y1 + 43.5, layer='레이져')
                rectangle(doc, x1 + EXsizex3 - 45, y4 - 26.5, x1 + EXsizex3 + 45, y4 - 43.5, layer='레이져')
                ...
                dim(doc, x3, y3, x4, y4, 150, direction="up", option='reverse', localdimstyle="0.2 JKW")
                # 하부 전체치수
                dim(doc, x1 + EXsizex3 - 45, y1 + 26.5, x1, y1, 100, direction="down", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x1 + EXsizex3 + 45, y1 + 26.5)
                dimcontinue(doc, x2, y2)
            ...
            ###################################################
                ...
                if CD > 1600: 
                    coverplatex4 = EXsizey2/2 - 4
                else:
                    coverplatex4 = x2 - x1 - 4
                ...
                textstr = f"Mat.Spec : SPCC 1.6t"    
                draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
                textstr = f"Size : 130 x {coverplatex4:.1f}mm"    
                draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
                ...
                frameXpos = frameXpos + TargetXscale + 600     
                
    #################################################################################################
    # 9page EX Bracket (비상구출구 있을때만 작도)
    #################################################################################################
        if emergencytype == "있음": 
            ...
            ###################################################
                rx1 = frameXpos + 200
                ry1 = frameYpos + 1100 * frame_scale + 200
                ...
                insert_block(x1, y1, "Ceiling_EXIT_bk_bottom")
                insert_block(x10, y10, "Ceiling_EXIT_bk_bottom_R")
                    
                text = "마킹선"
                dim_leader(doc, x6, y6 + 30, x6 + 50, y6 + 10 + 70, text, direction="leftToright", distance=13)
            
                rectangle(doc, x1 + emergencyRow/2 + 19 - 45, y15 - 24, x1 + emergencyRow/2 + 19 + 45, y15 - 7, layer='레이져')
                line(doc,  x1 + emergencyRow/2 + 19 - 45, y15 - 24, x1 + emergencyRow/2 + 19 + 45, y15 - 7, layer='6')
                line(doc,  x1 + emergencyRow/2 + 19 - 45, y15 - 7, x1 + emergencyRow/2 + 19 + 45, y15 - 24, layer='6')

                #마킹선
                line(doc, x5, y5, x5, y5 + 61, layer='6')
                line(doc, x6, y6, x6, y6 + 61, layer='6')
                
                #절곡라인  (히든선 : 정방향)      
                line(doc, x20, y20, x11, y11, layer='2')
                line(doc, x19, y19, x12, y12, layer='2')
                
                # 상부 전체치수
                dim(doc, x18, y18, x13, y13, 230, direction="up", option='reverse', localdimstyle="0.2 JKW")
                dim(doc, x1 + emergencyRow/2 + 19 - 45, y18, x18, y18, 150, direction="up", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x1 + emergencyRow/2 + 19 - 45 + 90, y18)
                dimcontinue(doc, x13, y13)
                # 하부 전체치수
                dim(doc, x4, y4, x1, y1, 100, direction="down", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x7, y7)
                dimcontinue(doc, x10, y10)
                ...
                # main Frame description    
                textstr = f"Part Name : EX Bracket"    
                draw_Text(doc, temp_x1, temp_y1, 30, str(textstr), layer='0')    
                textstr = f"Mat.Spec : SPCC 2.3T"    
                draw_Text(doc, temp_x1, temp_y1 - 60, 30, str(textstr), layer='0')    
                textstr = f"Size : 141 x {emergencyRow + 87}mm"    
                draw_Text(doc, temp_x1, temp_y1 - 120, 30, str(textstr), layer='0')        
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, temp_x1, temp_y1 - 180, 30, str(textstr), layer='0')    
            ...
            ###################################################
                ...
                x1 = rx1
                y1 = ry1
                x2 = x1 + emergencyRow + 87
                y2 = y1
                x3 = x2
                y3 = y2 + 50 - thickness
                x4 = x3
                y4 = y3 + 30 - thickness
                x5 = x4 - emergencyRow - 87
                y5 = y4
                x6 = x5
                y6 = y5 - 30 + thickness
                ...
                insert_block(x1, y1, "Ceiling_EXIT_bk_top")
                
                line(doc, x3, y3, x6, y6, layer='hidden')
                
                # 치수선
                dim(doc, x4, y4, x5, y5, 100, direction="up", localdimstyle="0.2 JKW")
                dim(doc, x4, y4, x2, y2, 160, direction="right", localdimstyle="0.2 JKW")
                dim(doc, x3, y3, x2, y2, 100, direction="right", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x4, y4)
                ...
                textstr = f"Mat.Spec : SPCC 2.3t"    
                draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
                textstr = f"Size : 76 x {emergencyRow + 87}mm"    
                draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, x1, y1 - 180, 30, str(textstr), layer='0')    
    ...
    #################################################################################################
        if emergencytype == "있음": 
            ...
            ###################################################
                rx1 = frameXpos + 200
                ry1 = frameYpos + 1100 * frame_scale + 250
                
                thickness = 1.5
                vcut = thickness / 2

                insert_block(rx1, ry1 + emergencyCol/2 + 60 + 54, "Ceiling_EX_cover_top")
                insert_block(rx1, ry1 + 54, "Ceiling_EX_cover_bottom")
                
                line(doc, rx1, ry1 + 54, rx1, ry1 + emergencyCol/2 + 60 + 54, layer="0")
                line(doc, rx1 - 1.6, ry1 + 54 + 1.6, rx1 - 1.6, ry1 + emergencyCol/2 + 60 + 54 - 1.6, layer="0")
                dim(doc, rx1, ry1 + 54, rx1, ry1 + emergencyCol/2 + 60 + 54, 100, direction="right", localdimstyle="0.2 JKW")

                x1 = rx1 + 200
                y1 = ry1
                x2 = x1 + emergencyRow + 20
                y2 = y1
                x3 = x2
                y3 = y2 + 20 - thickness
                x4 = x3
                y4 = y3 + 40 - thickness*2
                x5 = x4
                y5 = y4 + emergencyCol/2 + 60 - thickness*2
                x6 = x5
                y6 = y5 + 40 - thickness*2
                x7 = x6
                y7 = y6 + 20 - thickness
                x8 = x7 - emergencyRow - 20
                y8 = y7
                x9 = x8
                y9 = y8 - (20 - thickness)
                x10 = x9
                y10 = y9 - (40 - thickness*2)
                x11 = x10
                y11 = y10 - (emergencyCol/2 + 60 - thickness*2)
                x12 = x11
                y12 = y11 - (40 - thickness*2)
                ...
                text = "2-R5"
                dim_leader(doc, x8, y8, x8 - 50, y8 + 50, text, direction="rightToleft", distance=13)
            
                rectangle(doc, x10 + 100, y10, x10 + 160, y9, layer='레이져')
                rectangle(doc, x5 - 100, y5, x6 - 160, y6, layer='레이져')
                
                #절곡라인  (히든선 : 정방향)      
                line(doc, x3, y3, x12, y12, layer='hidden')
                line(doc, x4, y4, x11, y11, layer='hidden')
                line(doc, x5, y5, x10, y10, layer='hidden')
                line(doc, x6, y6, x9, y9, layer='hidden')
                
                # 상부 전체치수
                dim(doc, x8 + 100, y9, x8, y8, 100, direction="up", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x8 + 160, y9)
                dimcontinue(doc, x7 - 160, y9)
                dimcontinue(doc, x7 - 100, y9)
                dimcontinue(doc, x7, y7)
                # 하부 전체치수
                dim(doc, x2, y2, x1, y1, 50, direction="down", localdimstyle="0.2 JKW")
                # 절곡 치수
                dim(doc, x7, y7, x2, y2, 160, direction="right", option='reverse', localdimstyle="0.2 JKW")
                dim(doc, x3, y3, x2, y2, 100, direction="right", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x4, y4)
                dimcontinue(doc, x5, y5)
                dimcontinue(doc, x6, y6, option='reverse')
                dim(doc, x6, y6, x7, y7, 100, direction="right", localdimstyle="0.2 JKW")
        
                temp_x1 = x1 + emergencyRow/2 - 100
                temp_y1 = y1 - 200
            
                # main Frame description    
                textstr = f"Part Name : EX Cover"    
                draw_Text(doc, temp_x1, temp_y1, 30, str(textstr), layer='0')    
                textstr = f"Mat.Spec : SPCC 1.6T"    
                draw_Text(doc, temp_x1, temp_y1 - 60, 30, str(textstr), layer='0')    
                textstr = f"Size : {int(emergencyCol/2 + 168)} x {emergencyRow + 20}mm"    
                draw_Text(doc, temp_x1, temp_y1 - 120, 30, str(textstr), layer='0')        
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, temp_x1, temp_y1 - 180, 30, str(textstr), layer='0')    
            ...
            # 10page 비상구 커버 하
            ###################################################
                rx1 = frameXpos + 200 + 1400
                ry1 = frameYpos + 1100 * frame_scale + 350
                
                thickness = 1.5
                vcut = thickness / 2

                insert_block(rx1, ry1 + emergencyCol/2 - 40 + 54, "Ceiling_EX_cover_top")
                insert_block(rx1, ry1 + 54, "Ceiling_EX_cover_bottom")
                
                line(doc, rx1, ry1 + 54, rx1, ry1 + emergencyCol/2 - 40 + 54, layer="0")
                line(doc, rx1 - 1.6, ry1 + 54 + 1.6, rx1 - 1.6, ry1 + emergencyCol/2 - 40 + 54 - 1.6, layer="0")
                dim(doc, rx1, ry1 + 54, rx1, ry1 + emergencyCol/2 - 40 + 54, 100, direction="right", localdimstyle="0.2 JKW")
                ...
                y4 = y3 + 40 - thickness*2
                x5 = x4
                y5 = y4 + emergencyCol/2 - 40 - thickness*2
                x6 = x5
                y6 = y5 + 40 - thickness*2
                x7 = x6
                y7 = y6 + 20 - thickness
                ...
                y10 = y9 - (40 - thickness*2)
                x11 = x10
                y11 = y10 - (emergencyCol/2 - 40 - thickness*2)
                x12 = x11
                y12 = y11 - (40 - thickness*2) 
                ...
                def draw_arc(doc, start, end, radius, option=None, counter= None): # counter는 첫번째 arc를 그릴때는 line을 그리지 않기 위한 옵션
                    """ option='reverse'는 시계방향으로 점을 회전할때 이 것을 사용한다. """
                    if option == 'reverse' :                
                        line(doc, prev_x, prev_y, start[0], start[1], layer='레이져')                
                        add_arc_between_points(doc, end, start, radius)
                ...
                for i in range(1, lastNum + 1):
                    ...
                    if i == 1:
                        draw_arc(doc, (curr_x, curr_y + Radiusval ), (curr_x + Radiusval, curr_y), Radiusval, counter=1)
                        prev_x, prev_y = curr_x + Radiusval, curr_y        
                        ...
                        line(doc, prev_x, prev_y, curr_x, curr_y, layer="레이져")
                        prev_x, prev_y = curr_x, curr_y
                    # print(f" i {i} prev_x, prev_y  { prev_x},  {prev_y}")     
                        
                # print(f"  마지막 prev_x, prev_y  { prev_x},  {prev_y} , x1, y1+5 : {x1}, {y1+5}")                             
                line(doc, prev_x, prev_y, x1, y1+5  , layer="레이져")
                                    
                insert_block(x1 + emergencyRow/2 + 10, y1, "Ceiling_EX_cover_hole")
                insert_block(x1 + emergencyRow/2 + 10, y1, "Ceiling_EX_cover_hole_laser")
                
                text = "2-R5"
                dim_leader(doc, x1, y1, x1 - 50, y1 - 50, text, direction="rightToleft", distance=13)
                ...
                # 상부 전체치수
                dim(doc, x8, y8, x7, y7, 110, direction="up", localdimstyle="0.2 JKW")
                dim(doc, x8 + (emergencyRow + 20)/2, y8 - emergencyCol/2 + 40, x8, y8, emergencyCol/2 + 20, direction="up", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x7, y7)
                # 하부 전체치수
                dim(doc, x1 + emergencyRow/2 - 70, y1 + 20, x1, y1, 120, direction="down", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x2 - emergencyRow/2 + 70, y1 + 20)
                dimcontinue(doc, x2, y2)
                dim(doc, x1, y1, x1 + emergencyRow/2 - 103, y1, 160, direction="down", localdimstyle="0.2 JKW")
                dimcontinue(doc, x1 + emergencyRow/2 - 103 + 45, y1)
                dimcontinue(doc, x2, y2)
                # 절곡 치수
                dim(doc, x7, y7, x2, y2, 160, direction="right", option='reverse', localdimstyle="0.2 JKW")
                dim(doc, x3, y3, x2, y2, 100, direction="right", option='reverse', localdimstyle="0.2 JKW")
                ...
                textstr = f"Mat.Spec : SPCC 1.6T"    
                draw_Text(doc, temp_x1, temp_y1 - 60, 30, str(textstr), layer='0')    
                textstr = f"Size : {int(emergencyCol/2 + 68)} x {emergencyRow + 20}mm"    
                draw_Text(doc, temp_x1, temp_y1 - 120, 30, str(textstr), layer='0')        
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, temp_x1, temp_y1 - 180, 30, str(textstr), layer='0')    
            ...
            ###################################################
                rx1 = frameXpos + 200
                ry1 = frameYpos + 1100 * frame_scale + 250 - 800
                
                insert_block(rx1, ry1, "Ceiling_EXIT_bk")
                ...
                x1 = rx1 + 125
                y1 = ry1 - 15
                x2 = x1 + emergencyCol/2 + 54
                y2 = y1
                x3 = x2
                y3 = y2 + 18 - thickness
                x4 = x3
                y4 = y3 + 35 - thickness*2
                x5 = x4
                y5 = y4 + 18 - thickness
                x6 = x5 - (emergencyCol/2 + 54)
                y6 = y5
                x7 = x6
                y7 = y6 - 18 + thickness
                x8 = x7
                y8 = y7 - 35 + thickness*2
                ...
                #절곡라인  (히든선 : 정방향)      
                line(doc, x3, y3, x8, y8, layer='hidden')
                line(doc, x4, y4, x7, y7, layer='hidden')
                
                # 치수선
                dim(doc, x6, y6, x5, y5, 50, direction="up", localdimstyle="0.2 JKW")
                dim(doc, x5, y5, x2, y2, 160, direction="right", localdimstyle="0.2 JKW")
                dim(doc, x3, y3, x2, y2, 50, direction="right", option='reverse', localdimstyle="0.2 JKW")
                dimcontinue(doc, x4, y4, 100)
                dimcontinue(doc, x5, y5)
                ...
                textstr = f"Mat.Spec : SPCC 1.6t"    
                draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
                textstr = f"Size : 65 x {int(emergencyCol/2 + 54)}mm"    
                draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
                textstr = f"Quantity : {su*2} EA"    
                draw_Text(doc, x1, y1 - 180, 30, str(textstr), layer='0')    
            ...
            ###################################################
                ...
                rectangle(doc, x1 - 1500, y1 - 200, x1 + 1100, y1 + 350, layer='0')
                textstr = f"샤링작업"    
                draw_Text(doc, x1 - 300, y1 + 250, 60, str(textstr), layer='0')
                ...
                textstr = f"Mat.Spec : SPCC 1.6t"    
                draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
                textstr = f"Size : 65 x {int(emergencyCol/2 - 46)}mm"    
                draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
                textstr = f"Quantity : {su*2} EA"    
                draw_Text(doc, x1, y1 - 180, 30, str(textstr), layer='0')    
    ...
    #################################################################################################
    # 11page EX switch (비상구출구 있을때만 작도)
    #################################################################################################
        if emergencytype == "있음": 
            program = 1
            if program:         
                ...
                insert_block(rx1, ry1, "Ceiling_EXIT_switch_parts")
                insert_block(rx1, ry1, "Ceiling_EXIT_switch_parts_laser")
                
                # main Frame description    
                x1 = rx1 + 80
                y1 = ry1 + 130
                textstr = f"Part Name : 스위치 커버"    
                draw_Text(doc, x1, y1, 8, str(textstr), layer='0')
                textstr = f"Mat.Spec : SPCC 1.6t"    
                draw_Text(doc, x1, y1 - 15, 8, str(textstr), layer='0')
                textstr = f"Size : 233 x 59.5mm"    
                draw_Text(doc, x1, y1 - 30, 8, str(textstr), layer='0')
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, x1, y1 - 45, 8, str(textstr), layer='0')    
                
                x1 = rx1 + 80 + 430
                y1 = ry1 + 130
                textstr = f"Part Name : 스위치 레버"    
                draw_Text(doc, x1, y1, 8, str(textstr), layer='0')
                textstr = f"Mat.Spec : EGI 2.3t"    
                draw_Text(doc, x1, y1 - 15, 8, str(textstr), layer='0')
                textstr = f"Size : 87.5 x 15mm"    
                draw_Text(doc, x1, y1 - 30, 8, str(textstr), layer='0')
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, x1, y1 - 45, 8, str(textstr), layer='0')    
                ...
                x1 = rx1 + 80 + 430
                y1 = ry1 + 130 + 135
                textstr = f"Part Name : 스위치 조립판"    
                draw_Text(doc, x1, y1, 8, str(textstr), layer='0')
                textstr = f"Mat.Spec : EGI 1.6t"
                draw_Text(doc, x1, y1 - 15, 8, str(textstr), layer='0')
                textstr = f"Size : 77 x 50mm"    
                draw_Text(doc, x1, y1 - 30, 8, str(textstr), layer='0')
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, x1, y1 - 45, 8, str(textstr), layer='0')    
    ...
    #################################################################################################
        if LHtype == "LH":
            ...
            if program:         
                ...
                insert_block(rx1 + 120, ry1 + 350, "eiling_LH_Fancover")
                insert_block(rx1 + 120, ry1 + 350, "Ceiling_LH_Fancover_laser")
                
                # main Frame description    
                x1 = rx1 + 2000
                y1 = ry1 + 1300
                textstr = f"Part Name : 팬커버 상판"    
                draw_Text(doc, x1, y1, 30, str(textstr), layer='0')
                textstr = f"Mat.Spec : SPCC 1.6t"    
                draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
                textstr = f"Size : 496.2 x 440mm"    
                draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
                textstr = f"Quantity : {su} EA"    
                draw_Text(doc, x1, y1 - 180, 30, str(textstr), layer='0')    
                ...
                x1 = rx1 + 2000
                y1 = ry1 + 600
                textstr = f"Part Name : 팬커버 마구리"    
                draw_Text(doc, x1, y1, 30, str(textstr), layer='0')
                textstr = f"Mat.Spec : SPCC 1.6t"    
                draw_Text(doc, x1, y1 - 60, 30, str(textstr), layer='0')
                textstr = f"Size : 145.6 x 158mm"    
                draw_Text(doc, x1, y1 - 120, 30, str(textstr), layer='0')
                textstr = f"Quantity : {su*2} EA"    
                draw_Text(doc, x1, y1 - 180, 30, str(textstr), layer='0')    
...
def main():
    ...
    global headertype, emergencytype, emergencyCol, emergencyRow, LHtype, entrytype, fan_su
    ...
    for file_path in xlsm_files:
        ...
        if not LCframeMaterial or (isinstance(LCframeMaterial, str) and LCframeMaterial.isspace()):
            if lctype == '026':
                LCframeMaterial = 'SPCC 1T'
            else:
                LCframeMaterial = 'SPCC 1.2T'        

        # LCplateMaterial이 중판 공백이나 None일 경우 'SPCC 1.2T'를 할당
        if not LCplateMaterial or (isinstance(LCplateMaterial, str) and LCplateMaterial.isspace()):
            LCplateMaterial = 'SPCC 1.2T'        
...
if __name__ == '__main__':
    ...
    #exe 만드는 명령어 : pyinstaller --add-data "data;data" --add-data "ceiling_excel;ceiling_excel" --noconfirm mirae_ceiling_250717.py