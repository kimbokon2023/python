...
# OP_raw, EE_raw, AS_raw, emergencyRow, emergencyCol, fan_su, fantop, fanside, ceilingdivision=0
...
def MainCeiling():   
    ...
    if program:    
        ...
        line(doc, x1, y1 + 30, x2, y1 + 30, layer="0")
        line(doc, x1, y1 + 30 - 2, x2, y1 + 30 - 2, layer="0")
        line(doc, x1, y2 - 30, x2, y2 - 30, layer="0")
        line(doc, x1, y2 - 30 + 2, x2, y2 - 30 + 2, layer="0")
        line(doc, x1 + 30, y1 + 30, x1 + 30, y2 - 30, layer="0")
        line(doc, x1 + 30 - 2, y1 + 30, x1 + 30 - 2, y2 - 30, layer="0")
        line(doc, x2 - 30, y1 + 30, x2 - 30, y2 - 30, layer="0")
        line(doc, x2 - 30 + 2, y1 + 30, x2 - 30 + 2, y2 - 30, layer="0")
        ...
        for x in frontPanel[:-1] :
            ...
            dim(doc, x1 + sum, y1, x1 + sum - x, y1, 150,  direction="down", localdimstyle="0.2 JKW")    
        ...
        for x in rearPanel[:-1] :
            ...
            dim(doc, x1 + sum, y2, x1 + sum - x, y2, 150,  direction="up", localdimstyle="0.2 JKW")    
        ...
        for y in sidePanel[:-1] :
            ...
            dim(doc, x1, y1 + sum, x1, y1 + sum - y, 150,  direction="left", localdimstyle="0.2 JKW")    
            dimcontinue(doc , x1, y1 + sum)
        dimcontinue(doc ,x1, y2 )
        ...
        # print(f" Topholey: {Topholey}")
        print(f" Bottomy: {Bottomy}")

        for x in Topholex:
            circle_cross(doc, x1 + 25 + x, y1 + 25 + Topholey, 13, layer='0') 
            
        for x in Bottomx:
            insert_block( abs_x + x, abs_y + Bottomy, "ceiling_LChole") 
        ...
        # circle_cross 함수 호출 시 리스트 길이를 확인하여 오류 방지
        for i in range(num_holes):
            circle_cross(doc, x1 + 25 + leftholex[i], y1 + 25 + vertical_holey[i], 11, layer='레이져')
            circle_cross(doc, x1 + 25 + rightholex[i], y1 + 25 + vertical_holey[i], 11, layer='레이져')
            # print(f"leftholex x좌표: {leftholex[i]} abs_x좌표 {abs_y }  vertical_holey : {vertical_holey[i]}")  
        
        #하부 가로 긴 브라켓
        insert_block(x1 + 31, y1 + 252.5, "Ceiling_Bracket_horizontal_holeL")
        insert_block(x2 - 31, y1 + 252.5, "Ceiling_Bracket_horizontal_holeR")
        rectangle(doc, x1 + 31, y1 + 187.5, x2 - 31, y1 + 205.5, layer='2')    
        rectangle(doc, x1 + 31, y1 + 299.5, x2 - 31, y1 + 317.5, layer='2')    
    ...
    #     dim(doc, abs_x + 85 , abs_y + 85 , abs_x + 85 , abs_y + CD - 85, 360, text_height=0.30,  direction="left") 
        ...
        dim(doc,  x2, y2, x1, y1,  210, text_height=0.32, direction="right")     
...
def main():
    ...
    global frontPanel, rearPanel, sidePanel, slotposition 
