...
def lc032():   
    ...
    circle_cross(doc, circle3dot2x5 , circle3dot2y5 , 3.2 , layer='레이져', color='0')     
    circle_cross(doc, circle3dot2x6 , circle3dot2y6 , 3.2 , layer='레이져', color='0')   
    circle_cross(doc, circle3dot2x7 , circle3dot2y7 , 3.2 , layer='레이져', color='0')   
...
def lc031():   
    ...
    BasicYscale = 1715
    if CW > 1600:
        TargetXscale = 5400 + (CW - 1600)*2.2
    else:
        TargetXscale = 5400 + (CW - 1600)
    if CD > 1500:        
        TargetYscale = 3300 + (CD - 1500)*2.2
    else:
        TargetYscale = 3300 + (CD - 1500)
    ...
    TargetXscale = 3000 + (CD - 1500)  # 기본 모형이 1450
    TargetYscale = 1800 
    if(TargetXscale/BasicXscale > TargetYscale/BasicYscale ):
        frame_scale = TargetXscale/BasicXscale
    else:
    ...
    BasicXscale = 2813
    BasicYscale = 1765
    TargetXscale = 2268 + (CW - 1600) * 1.2 if(CW>1600) else 2300
    TargetYscale = 1382 + (CD - 1500) * 1.3 if(CD>1500) else 1500 
    if(TargetXscale/BasicXscale > TargetYscale/BasicYscale ):
        frame_scale = TargetXscale/BasicXscale
    else:
        frame_scale = TargetYscale/BasicYscale    
...
def lc035():   
    ...
    if program:
        ...
        # 10파이 지시선
        dim_leader_line(doc, baseX, baseY , baseX + 50 ,baseY - 30, f"{len(lc035_vertical_holes)} -%%C11 Hole (M8 Pop Nut)")       
...
@Gooey(encoding='utf-8', program_name='미래기업 천장 & LIGTH CASE 자동작도', tabbed_groups=True, navigation='Tabbed', show_success_modal=False,  default_size=(800, 600))
def main():
    global args  #수정할때는 global 선언이 필요하다. 단순히 읽기만 할때는 필요없다.    
    global start_time
    global workplace, drawdate, deadlinedate, lctype, secondord, text_style, exit_program, Vcut_rate, Vcut, program_message, formatted_date, text_style_name
    ...
    for file_path in xlsm_files:
        ...
        # LCframeMaterial이 공백이나 None일 경우 'SPCC 1.2T'를 할당
        if not LCframeMaterial or LCframeMaterial.isspace():
            LCframeMaterial = 'SPCC 1.2T'

        # LCplateMaterial이 공백이나 None일 경우 'SPCC 1.2T'를 할당
