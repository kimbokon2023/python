...
# def frontPanel(sheet, row=19, start_col=4, end_col=25, spacing=10):
#     """
#     엑셀 데이터를 읽어 X 좌표를 계산하고, 해당 위치에 블록을 삽입하는 함수.

#     :param sheet: 엑셀 시트 객체
#     :param row: 데이터를 읽어올 행 번호 (기본값: 19)
#     :param start_col: 시작 열 번호 (기본값: 4)
#     :param end_col: 끝 열 번호 (기본값: 25)
#     :param spacing: 각 패널 사이의 간격 (기본값: 10)
#     """
#     # X 좌표 계산
#     x_coords = [
#         i * spacing for i in range(
#             len([
#                 sheet.cell(row=row, column=col).value
#                 for col in range(start_col, end_col + 1)
#                 if sheet.cell(row=row, column=col).value not in (0, None)
#             ])
#         )
#     ]
...
#본천장 홀간격계산
def calculate_Mainceiling_position(sheet, row=19, start_col=4, end_col=25, inputCD=600):
    """
    엑셀에서 X 좌표를 가져와서 천장 패널의 좌표를 계산하는 함수.

    :param sheet: 엑셀 시트 객체
    :param row: 데이터를 읽어올 행 번호 (기본값: 19)
    :param start_col: 시작 열 번호 (기본값: 4)
    :param end_col: 끝 열 번호 (기본값: 25)
    :param inputCD: 천장 깊이 (기본값: 600)
    :return: 좌표를 포함한 딕셔너리
    """
    ...
    # Y 좌표 계산
    Topholey = int(inputCD - 100)  # 윗쪽 구멍 Y 좌표
    Bottomy = 100  # 아랫쪽 구멍 Y 좌표

    # X 좌표 적용
    Topholex = x_coords  # 윗쪽 X 좌표 리스트
    Bottomx = x_coords  # 아래쪽 X 좌표 리스트
...
def MainCeiling():   
    ...
    if program:    
        ...
        x2 = abs_x + CW
        y2 = abs_y + CD    
        rectangle(doc, x1, y1, x2, y1 + (CD - 1000), layer='0')    
        rectangle(doc, x1, y1 + (CD - 1000), x2, y2, layer='0')    
        ...
        for x in frontPanel[:-1] :
            # insert_block(x1 + x, y1 + slotposition, "8x20 slot")
            sum = sum + x
            insert_block(x1 + sum, y1 + slotposition, "8x16 slot")
        
        # for x in frontPanel:
        # insert_block(doc, x, Y_POSITION, DIAMETER)   
...
def main():
    ...
    for file_path in xlsm_files:
        ...
        print(f"entrytype : {entrytype}")  # 확인용 출력
        print(f"doortype : {doortype}")  # 확인용 출력
        print(f"capacity : {capacity}")  # 확인용 출력
        print(f"OP_raw : {OP_raw}")  # 확인용 출력
        print(f"EE_raw : {EE_raw}")  # 확인용 출력
        print(f"AS_raw : {AS_raw}")  # 확인용 출력
        print(f"baseMaterial : {baseMaterial}")  # 확인용 출력
        print(f"LHtype : {LHtype}")  # 확인용 출력
        print(f"emergencytype : {emergencytype}")  # 확인용 출력
        print(f"emergencyRow : {emergencyRow}")  # 확인용 출력
        print(f"emergencyCol : {emergencyCol}")  # 확인용 출력
        print(f"slotposition : {slotposition}")  # 확인용 출력
        print(f"sillspace : {sillspace}")  # 확인용 출력
        print(f"headertype : {headertype}")  # 확인용 출력
        print(f"fan_su : {fan_su}")  # 확인용 출력
        print(f"fantop : {fantop}")  # 확인용 출력
        print(f"fanside : {fanside}")  # 확인용 출력
        print(f"ceilingdivision : {ceilingdivision}")  # 확인용 출력
