...
## 2024/11/26 로그파일 코드 삽입, SO 오류부분 수정 변수 미할당 
...
def log_login():
    ...
    url = f"http://8440.co.kr/autopanel/savelog.php?company=미래기업 쟘덧방 자동작도&content={workplace} 현장"
...
def execute_3up():    
    ...
    if True:
        ...
        for idx, row in enumerate(data_2d): 
            ...
            if(worker=="손상민"  or worker == "손상민(일체)" and G_Setvalue > 0) :
                # 상부 돌출
                Top_pop1 = G_Setvalue
                Top_pop2 = G_Setvalue
        ...
        for idx, row in enumerate(data_2d): 
            ...
            if True:
                ...
                if(OP>0):          
                    ...
                    # Vcut 현장 작업용 치수선 표시
                    dim_vertical_left(doc, X2, Y2, X4, Y4, 100, 'dim', text_height=0.22, text_gap=0.01)
...
def execute_fourcowide():     
    ...
    for idx, row in enumerate(data_2d): 
        ...
        if(U>0 and G>0 and MH1 == MH2 ) :   # MH가 양쪽 같을때만 표시함  
