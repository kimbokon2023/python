...
def numprint(doc, x1, y1, text=None, layer=None):
    ...
    draw_Text(doc, x1-35, y1-12.5, 25, text, layer=layer)
...
def draw_page1():     
    ...
    rx = abs_x + P1_width + OP - panel_width + column_BottomHoleHorizontal                       
    ...
    insert_frame(frameXpos-784 , frameYpos  , frame_scale, "drawings_frame", "CAGE ASY", f"CH:{CH}, KP:{KH}+{FH}, WH:{CPH}", "drawing number")   
    ...
    x2 = x1 + panel_wing + column_thickness + column_width + P1_width - 12.5 - br*6
    y2 = y1    
    x3 = x2 + 12.5 + panel_width + panel_wing - br*4  # 12.5는 끝기준 장공 위치
    y3 = y2 
    x4 = x3
    y4 = y3 + FH
    x5 = x4 
    ...
    x6 = x5 
    y6 = y5 + CH - OPH - FH 
    x7 = x6 + (12.5 + panel_width + panel_wing - br*4 ) * - 1
    y7 = y6 
    x8 = x7 - ( panel_wing + column_thickness + column_width + P1_width + panel_width + panel_wing - br*10 ) + (panel_wing+column_thickness+46) - (12.5 + panel_width + panel_wing - br*4 )* -1
    y8 = y7 
    x9 = x8 
    y9 = y8  - CH + OPH + FH  + 40
    x10 = x1 
    ...
    # COP 타공
    ##################################    
    tx1 = x1 + panel_wing + column_thickness + column_width + COP_centerdistance - br*6 - COP_centerdistance/2
    ty1 = y1 + KPH + COP_bottomHeight 
    tx2 = tx1 + COP_centerdistance
    ty2 = ty1
    tx3 = tx2
    ty3 = ty2 + COP_height
    tx4 = tx3 - COP_centerdistance
    ty4 = ty3
    ...
    x1 = rx 
    y1 = ry
    x2 = x1 + 12.5 + panel_width + panel_wing - br*4  # 12.5는 끝기준 장공 위치
    y2 = y1    
    x3 = x2 + panel_wing + column_thickness + column_width + P11_width - 12.5 - br*6
    y3 = y2 
    x4 = x3
    y4 = y3 + FH
    x5 = x4 
    ...
    x8 = x7 
    y8 = y7 + CH - OPH - FH  - 40 
    x9 = x1 + (12.5 + panel_width + panel_wing - br*4 ) 
    y9 = y8  
    x10 = x1 
    y10 = y9 
    x11 = x10 
    ...
    ###################################################
    # 4page HOP 보강 'ㄷ'자 보강 좌측 하단 bracket
    ###################################################
...
@app.get("/")
async def root():
    return PlainTextResponse("Welcome to the DXF Converter API. Use POST /upload/ to upload an Excel file.")

@app.post("/upload/")
async def create_upload_file(file: UploadFile = File(...)):
    try:
        path = "temp_excel.xlsm"
        with open(path, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
        
        # 파일 처리 로직 호출
        output_file_path = process_excel_to_dxf(path)
        return FileResponse(path=output_file_path, filename="output.dxf")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"An error occurred: {str(e)}")

def process_excel_to_dxf(input_file_path):
    global args  #수정할때는 global 선언이 필요하다. 단순히 읽기만 할때는 필요없다.    
    global start_time
