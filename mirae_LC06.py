...
def dim(doc, x1, y1, x2, y2, dis, text_height=0.22, text_gap=0.05, direction="up", option=None, startoption=None, text=None):
    global saved_DimXpos, saved_DimYpos, saved_text_height, saved_text_gap, saved_direction, dimdistance, dim_horizontalbase, dim_verticalbase
...
def calculate_lc_position_008_A2_CW_P(inputCW, inputCD):#CW 팝너트 홀
    # Calculate Y-coordinates for top and bottom holes
...
# def calculate_lc_position_008_A2_RIGHT(inputCW, inputCD):#오른쪽 위아래 홀
#     # Calculate Y-coordinates for top and bottom holes
#     Topholey = (int)(inputCD - 13)
#     Bottomy = 13
...
def calculate_lc_position_008_A2_CD_P(inputCW, inputCD):#CD 팝너트 홀
    # Calculate Y-coordinates for top and bottom holes
    Leftx = 13 
...
# def calculate_lc_position_008_A2_CD_P(inputCW, inputCD):#아래쪽 좌우 홀
#     # Calculate Y-coordinates for top and bottom holes
#     Leftx = 13 
#     Rightx = (int)(inputCW - 13)
...
#     y2 = int((inputCD/2-firstYgap*2)/3*10)/10 + firstYgap
#     y3 = int((inputCD/2-firstYgap*2)*2/3*10)/10 + firstYgap
#     y4 = int((inputCD/2 - firstYgap)*10)/10 + firstYgap
...
def calculate_lc_position_008_A2_ASSY_horizontal (inputCW, inputCD):
...
def calculate_lc_position_008_A2_ASSY_vertical  (inputCW, inputCD):
    ...
    y1 = firstYgap
    base = (inputCD / 2 - firstYgap * 2) / 3  # 나누기 연산 후 처리
...
def calculate_lc_position_008_A2_FRAME_CD(inputCW, inputCD):
    # Calculate Y-coordinates for top and bottom holes
    Bottomy = 10
    ...
    # Calculating all x-coordinates
    calx1 = int(firstXgap)
    calx2 = int((inputCD/2 - firstXgap-SecondXgap)/3*10)/10 + firstXgap
    calx3 = int((inputCD/2 - firstXgap-SecondXgap)*2/3*10)/10 + firstXgap
    calx4 = int((inputCD/2 - firstXgap-SecondXgap)*10)/10 + firstXgap


    total = calx2 + calx3 + calx4
    # total의 소수 첫째자리(10분위)가 0이 아니면 x3에 0.1을 추가
    if int(total * 10) % 10 != 0:
        calx3 += 0.1    
...
# def calculate_lc_position_008_A2_FRAME_CW(inputCW, inputCD):
#     # Calculate Y-coordinates for top and bottom holes
#     Bottomy = 10
...
#     # Calculating all x-coordinates
#     x1 = firstXgap
#     x2 = int(inputCW/2 - firstXgap-SecondXgap)/3 + firstXgap
#     x3 = int(inputCW/2 - firstXgap-SecondXgap)*2/3 + firstXgap
#     x4 = int(inputCW/2 - firstXgap-SecondXgap) + firstXgap


#     total = x2 + x3 + x4
#     # total의 소수 첫째자리(10분위)가 0이 아니면 x2에 0.1을 추가
#     if int(total * 10) % 10 != 0:
#         x2 += 0.1    
...
def lc026():     
    ...
    draw_Text(doc, xx+50, yy+75 , 12, str(textstr), layer='0')
    textstr = f"W{CW} x D{CD}"    
    ...
    # 장공/단공 70
    insert_block(abs_x + 150 , abs_y  + 275 , "circle70")    
    insert_block(abs_x + CW - 150 , abs_y  + 275 , "circle70")    
    insert_block(abs_x + CW - 150 , abs_y  + CD - 275 , "circle70")    
    insert_block(abs_x + 150 , abs_y  + CD - 275 , "circle70")    
    # 장공/단공 20
    ...
    textstr = f"CAR INSIDE"    
    draw_Text(doc, xx+50*2, yy+75*2 , 24, str(textstr), layer='0')
    textstr = f"W{CW} x D{CD}"    
    ...
    # 회색선 다량생성    
    line(doc, x1, y1+12.5+2.75, x2, y1+12.5+2.75,  layer='2')    # 0 선  
    line(doc, x1, y1+37.5, x2, y1+37.5,  layer='2')    # 0 선  
    line(doc, x1, y1+12.5+2.75+50, x2, y1+12.5+2.75+50,  layer='2')    # 0 선      
    for i in range(1, 13 + 1):
        line(doc, x1, y1+12.5+2.75+i*1.5, x2, y1+12.5+2.75+i*1.5,  layer='2')    # 0 선  
        line(doc, x1, y1+50+12.5+2.75+i*1.5, x2, y1+50+12.5+2.75+i*1.5,  layer='2')    # 0 선  
...
def lcN20():    
    ...
    dim(doc, x1, y1, x2, y2, 80 + 75, text_height=0.22, text_gap=0.07, direction="down")
    dim(doc, x2, y2, x7, y7, 80 + 75, text_height=0.22, text_gap=0.07, direction="down")
    dim(doc, x7, y7, x8, y8, 80 + 75, text_height=0.22, text_gap=0.07, direction="down")
...
def lc032():   
    ...
    x2 = abs_x + CW - 345
    y2 = y1    
    dim(doc,  x2, y2,  x1, y1,153.5 + 2.75 , text_height=0.30, text_gap=0.07, direction="down")
    x1 = abs_x + 25
    y1 = abs_y + 36
    x2 = abs_x + CW - 25
    y2 = y1        
    ...
    circle3dot2x5 =  x18 + 375
    circle3dot2y5 =  y18 - 29.7 - 7
    circle3dot2x6 =  x17 - 375
    ...
    # 상부 치수선
    dim(doc,  x1, y1,verticalLinex1, verticalLiney1, 75 , text_height=0.22, text_gap=0.07, direction="up")
...
def lc031():   
    ...
    textstr = f"({watt})"            
    draw_Text(doc, 756, 310 , 18, str(textstr), layer='0')    
...
def lc008_A():   
    ...
    if program:
        ...
        # 흰색선
        line(doc, abs_x + 75 , abs_y , abs_x + CW - 150 + 75  ,abs_y , layer='0')
        line(doc, abs_x + 75 + 1.2 , abs_y + 1.2 , abs_x + CW - 150 + 75 -2.4  ,abs_y + 1.2 , layer='0')
    ...
    if program:    
        ...
        y2 = abs_y + CD - height_insideGap
        rectangle(doc, x1, y1, x2, y2, layer='0') 
        x1 = abs_x + CW - 85 - ribGap
        x2 = x1 + ribGap
        rectangle(doc, x1, y1, x2, y2, layer='0') 
        
        # 상하좌우1.6T hidden 선
        line(doc, abs_x + 85+30-1.6, abs_y + 85,  abs_x + 85+30-1.6, abs_y + CD - 85, layer="Hidden") #좌
        line(doc, abs_x + CW-85-30+1.6, abs_y + 85,  abs_x + CW-85-30+1.6, abs_y + CD - 85, layer="Hidden") #우
        line(doc, abs_x + 115, abs_y + CD -115+1.6,  abs_x + CW-115, abs_y + CD - 115+1.6, layer="Hidden") #상
        line(doc, abs_x + 115, abs_y + 115-1.6,  abs_x + CW-115, abs_y + 115-1.6, layer="Hidden") #하
        
        #내부 가로폭치수
        dim(doc, abs_x + 115-1.6, abs_y + CD -115+1.6,  abs_x + CW-115+1.6, abs_y + CD - 115+1.6, 120,  text_height=0.50, direction="down")
    ...
    # }        
        ...
        # print(f"LC 008_A 라이트홀 위치 : {lc_positions}")
        # print(f"LC 008_A 탑 홀 개수 : {len(lc_positions['Topholex'])}")
        ...
        for x in Middlex:
            circle_cross(doc, abs_x + x, abs_y + Middley, 11, layer='레이져')            
        ...
        y2 = y1    
        textstr =  f"CAR INSIDE(CW) = {CW}"
        dim(doc, x1 , y1 , x2  , y2, 300,  text_height=0.32, text_gap=0.07, direction="up", text = textstr)    

        x1 = abs_x 
        y1 = abs_y + CD
        x2 = abs_x + CW
        y2 = y1    
        dim(doc, x1 + Xgap , y1 - Xgap, x2 - Xgap , y2 - Xgap, 310,  text_height=0.32, text_gap=0.07, direction="up") 
        
        
        dim(doc, x1, y1, x1+Xgap, y1-Xgap, 150,  text_height=0.30, text_gap=0.07, direction="up")    
        dim(doc, x1+Xgap, y1-Xgap, x1+CW/2, y2-Xgap, 235,  text_height=0.30, text_gap=0.07, direction="up")    
        dim(doc, x2-Xgap, y1-Xgap, x1+CW/2, y2-Xgap, 235,  text_height=0.30, text_gap=0.07, direction="up")    
        dim(doc, x2-Xgap, y1-85, x2, y2, 235,  text_height=0.30, text_gap=0.07, direction="up")
        
        dim(doc, x1, y1, x1+100, y1-100, 75,  text_height=0.30, text_gap=0.07, direction="up")    
        dim(doc, x1+100, y1-100, x1+CW/2, y2-100, 175, text_height=0.30, text_gap=0.07, direction="up")    
        dim(doc, x2-100, y1-100, x1+CW/2, y2-100, 175, text_height=0.30, text_gap=0.07, direction="up")    
        dim(doc, x2-100, y1-100, x2, y2, 175,  text_height=0.30, text_gap=0.07, direction="up")    
        ...
        # 좌측 치수
        last_ypos = 100
        dim(doc, abs_x  , abs_y , abs_x + 100 , abs_y +  last_ypos, 120, text_height=0.30,  direction="left") 
        for i, y_coord in enumerate(sorted_y_coords):        
            # print(f"세로 홀의 y 좌표: {y_coord}")
            if i>0:
                dimcontinue(doc,  abs_x + 100 , abs_y +  y_coord)
                last_ypos = y_coord
        dimcontinue(doc,  abs_x , abs_y + CD , distance = 120, option='reverse')             
        
        dim(doc, abs_x  , abs_y , abs_x + 85 , abs_y + 85, 195, text_height=0.30,  direction="left") 
        dim(doc, abs_x + 85  , abs_y + 85 , abs_x + 100 , abs_y + CD/2, 280, text_height=0.30,  direction="left") 
        dim(doc, abs_x + 85  , abs_y + CD - 85 , abs_x + 100 , abs_y + CD/2, 280, text_height=0.30,  direction="left") 
        dim(doc, abs_x , abs_y + CD , abs_x + 85 , abs_y + CD - 85, 195, text_height=0.30,  direction="left") 
        
        dim(doc, abs_x + 85 , abs_y + 85 , abs_x + 85 , abs_y + CD - 85, 360, text_height=0.30,  direction="left") 
    ...
    if program:
        ...
        # 흰색선 삽입
        line(doc, x + 100,  y - 75,  x + 100, y1 + 75, layer="0")
        line(doc, x + 100 - 1.2,  y - 75 - 1.2,  x + 100 - 1.2, y1 + 75 + 1.2, layer="0")
        line(doc, x + 70,  y - 115,  x + 70, y1 + 115, layer="0")
        

        dim(doc, x + 100 , y - 75,  x + 100, y1 + CD/2,  300, text_height=0.3, direction="right")
        dim(doc, x + 100 , y1 + 75,  x + 100, y1 + CD/2,  300, text_height=0.3, direction="right")
    ...
    if program:    
        ...
        # #테두리
        # rectangle(doc, x1,  y2 - 25,  x1 + ICW/2-1.2, y2 - 1.2, layer='Hidden')#상좌
        # rectangle(doc, x2,  y2 - 25,  x1 + ICW/2+1.2, y2 - 1.2, layer='Hidden')#상우
        # rectangle(doc, x1,  y1 + 25,  x1 + ICW/2-1.2, y1 + 1.2, layer='Hidden')#하좌
        # rectangle(doc, x2,  y1 + 25,  x1 + ICW/2+1.2, y1 + 1.2, layer='Hidden')#하우
        
        # rectangle(doc, x1,  y1 + 25.5,  x1 + 25, y2 - 25.5, layer='Hidden')#좌
        # rectangle(doc, x2,  y1 + 25.5,  x2 - 25, y2 - 25.5, layer='Hidden')#우
        # line(doc, x1 + 1.2,  y1 + 25.5,  x1 + 1.2, y2 - 25.5, layer="Hidden")#좌
        # line(doc, x2 - 1.2,  y1 + 25.5,  x2 - 1.2, y2 - 25.5, layer="Hidden")#우
        ...
        lc_positions = calculate_lc_position_008_A2_CW_P(inputCW, inputCD)
    ...
    # }        
        ...
        for x in Topholex:
            circle_cross(doc, temp_x + x, temp_y + Topholey, 9, layer='레이져')    
            circle_cross(doc, temp_x + x, temp_y + Bottomy, 9, layer='레이져')          
    ...
    #     Bottomy = lc_positions['Bottomy']    
        ...
        lc_positions = calculate_lc_position_008_A2_CD_P(inputCW, inputCD)
        ...
        for y in Lefty:
            circle_cross(doc, temp_x + Leftx , temp_y + y, 9, layer='레이져')    
            circle_cross(doc, temp_x + Rightx , temp_y + y, 9, layer='레이져')            
        
                
        # lc_positions = calculate_lc_position_008_A2_CD_P(inputCW, inputCD)

        # # print(f"lc_positions 리스트 : {lc_positions}")
        ...
        #의장면 조립홀 간격별 선 그리기
        lc_positions = calculate_lc_position_008_A2_ASSY_horizontal(inputCW, inputCD)
        ...
        lc_positions = calculate_lc_position_008_A2_ASSY_vertical(inputCW, inputCD)
        ...
        msp.add_lwpolyline(points, dxfattribs={"const_width": 10})
        ...
        textstr = f"LED BAR = {CW - 250}"        
        dim(doc,  x1 + 125, y2 - 135, x2 - 125, y2 - 135,  200, text_height=0.30, direction="up", text = textstr)                 
        dim(doc,  x2 - 125, y1 + 150, x2 - 125, y2 - 150,  200, text_height=0.30, direction="right", text = textstr)                 
        ...
        frameXpos = frameXpos + TargetXscale + 500
    ...
    if program:
        ...
        x = abs_x + (CW-150)/4 + 75 + 1500
        y = abs_y 
        insert_block(x , y , "lc008_A_Halogen")
        
        x = abs_x + CW-((CW-150)/4 + 75) + 1500
        insert_block(x , y , "lc008_A_Halogen")
        ...
        temp_y = abs_y
        # 흰색선
        line(doc, temp_x + 75 , temp_y , temp_x + CW - 150 + 75  ,temp_y , layer='0')
        line(doc, temp_x + 75 + 1.2 , temp_y + 1.2 , temp_x + CW - 150 + 75 -2.4  ,temp_y + 1.2 , layer='0')
        line(doc, temp_x + 100 , temp_y + 30 , temp_x + CW - 100  ,temp_y + 30 , layer='0')
    ...
    if program:
        ...
        textstr = f"Car Inside (CD) = {CD}"
        dim(doc, x, y, x1 , y1,  200, text_height=0.3, direction="left", text=textstr)
    ...
    if program:
        x = abs_x + CW*2.1
        y = abs_y + CD
        insert_block(x , y , "lc008_A_assy_side_section_top_R")         
        x1 = x
        y1 = abs_y 
        insert_block(x1 , y1 , "lc008_A_assy_side_section_bottom_R")         
        x2 = x
        y2 = abs_y + CD/2 
        insert_block(x2-70 , y2 , "lc008_A_assy_side_section_midbolt_R")      
        ...
        dim(doc, x - 100 , y - 75,  x - 100, y1 + CD/2,  300, text_height=0.3, direction="left")
        dim(doc, x - 100 , y1 + 75,  x - 100, y1 + CD/2,  300, text_height=0.3, direction="left")

        textstr = f"Car Inside (CD) = {CD}"
        dim(doc, x, y, x1 , y1,  200, text_height=0.3, direction="right", text=textstr)
    ...
    if program:    
        ...
        dim(doc, x1 + 75, y2 - 75,  x1, y2,  250, direction="up", option="reverse")      
        dimcontinue(doc, x2 - 75, y2 - 75)
        dimcontinue(doc, x2, y2)
        dim(doc, x1 + 75, y2 - 75,  x1 + CW/2, y2 - 75,  200, direction="up")      
        dimcontinue(doc, x2 - 75, y2 - 75)
        #좌측 치수
        textstr =  f"CAR INSIDE(CD) = {CD}"    
        dim(doc,  x1, y1, x1, y2,  300, text_height=0.30, direction="left", text = textstr)                        
        dim(doc,  x1, y1, x1 + 75, y1 + 75,  200, direction="left")      
        dimcontinue(doc, x1 + 75, y2 - 75)
        dimcontinue(doc, x1, y2)
        dim(doc, x1 + 75, y1 + 75,  x1 + 75, y1 + CD/2,  150, direction="left")      
        dimcontinue(doc, x1 - 75, y2 - 75)  
    ...
    #     Bottomy = lc_positions['Bottomy']    
        ...
        for i, x in enumerate(Topholex):
            circle_cross(doc, temp_x + x, temp_y + Bottomy, 9, layer='레이져')           
            if i == 0:
                dim(doc, x1 + CW/2 + x -75, y1 , x1 + CW/2 + x -75-26, y1 , 150,  direction="down", option='reverse')    
            else:
                dimcontinue(doc ,x1 + CW/2 + x -75, y1 )
        ...
        # dimcontinue(doc ,x2, y1 + ICD/2-13 )
    ...
    if program:
        ...
        draw_circle(doc, x8 + 15 + (CD-200)/2, y8 - 15, 11, layer="레이져") 
        ...
        lc_positions = calculate_lc_position_008_A2_FRAME_CD(inputCW, inputCD)

        # print (f"함수에서 받은 자료 : {lc_positions}")
    ...
    # }        
        ...
        for i, x in enumerate(Bottomx):       
            circle_cross(doc, temp_x + x, temp_y + 10, 9, layer='레이져')    
            # print(f"x{i} : {x}")
            if i == 0:
                dim(doc, x1+x , y1 , x1, y1 , 100,  direction="down", option='reverse')    
            else:
                dimcontinue(doc , x1 + x, y1)
                
        for i, x in enumerate(Bottomx):       
            circle_cross(doc, temp_x + CD/2 + x -75, temp_y + 10, 9, layer='레이져')    
            # print(f"x{i} : {x}")
            if i == 0:
                dim(doc, x1 + CD/2 + x -75, y1 , x1 + CD/2 + x -75-26, y1 , 100,  direction="down", option='reverse')    
            else:
                dimcontinue(doc ,x1 + CD/2 + x -75, y1 )
        dimcontinue(doc ,x2, y2 )
        
        dim_leader_line(doc, x1 + (CD-156)/2+10, y1+10 , x1 + (CD-156)/2 + 50 ,y1+15+40, f"8-%%C9 Hole")       
        
        dim_leader_line(doc, x8 + (CD-156)/2, y8-15, x8 + (CD-156)/2 + 50 ,y8+ 30, f"3-%%C11 Holes (M8 Pop Nut)")       
        dim(doc, x8 + (CD-156)/2, y8 , x8 + (CD-156)/2 , y8-15, 100, direction="left")        
            
        dim_leader_line(doc, x1 + 77, y1 + 54, x1+77+30 ,y1+54+ 30, f"2-%%C20 Holes")       
        
        # 상부 전체치수
        dim(doc, x8+15, y8-15, x8, y8, 150, direction="up", option='reverse') 
        dimcontinue(doc, x7-15, y7-15)
        dimcontinue(doc, x7, y7)
        
        dim(doc, x10, y10, x8, y8, 210, direction="up", option='reverse') 
        dimcontinue(doc, x5, y5)
        dimcontinue(doc, x7, y7)
        
        dim(doc, x8+70 , y8-77, x8, y8, 310, direction="up", option='reverse') 
        dimcontinue(doc, x7-70, y7-77)
        dimcontinue(doc, x7, y7)
        
        dim(doc, x8 , y8, x12, y12, 280, direction="up", option='reverse') 
        dimcontinue(doc, x7, y7)
        dimcontinue(doc, x3, y3)

        # 좌측 치수선
        dim(doc,  x8, y8 , x9 , y9 , 150,  direction="left")           
        dimcontinue(doc, x12 , y12)
        dimcontinue(doc, x1 , y1, option='reverse')

        # 우측 치수선
        dim(doc,  x6, y6 , x7 , y7 , 160,  direction="right")         
        dimcontinue(doc,  x3, y3 )
        dimcontinue(doc,  x2, y2 )
           
        dim(doc,  x7, y7 , x2 , y2 , 220,  direction="right")        
        
        # #하단 치수선
        # dim(doc, x1+10 , y1+10 , x1, y1 , 100,  direction="down", option='reverse')    
        # dimcontinue(doc , x1 + 10+(CD/2-101)/3, y1+10 )
        # dimcontinue(doc , x1 + 10+(CD/2-101)*2/3, y1+10 )
        # dimcontinue(doc , x1 + 10+(CD/2-101), y1+10 )
        # dimcontinue(doc , x1 - 65+CD/2, y1+10 )
        # dimcontinue(doc , x1 - 65+CD/2+(CD/2-101)/3, y1+10 )
        # dimcontinue(doc , x1 - 65+CD/2+(CD/2-101)*2/3, y1+10 )
        # dimcontinue(doc , x1 - 65+CD/2+(CD/2-101), y1+10 )
        # dimcontinue(doc , x2 , y1+10 )
        
        dim(doc, x1 , y1 , x2, y2 ,150 ,  direction="down")    
        
        #내부 홀위치 치수선
        # dim(doc, x8+CD/2-85 , y8-15 , x8+CD/2, y8 ,50 ,  direction="left")    
        dim(doc, x2 , y2 , x2-10, y2+10 ,50 ,  direction="left")    
        dim(doc, x1 , y1 , x1+77, y2+54 ,130 ,  direction="right")    
        ...
        textstr = f"Mat.Spec : {LCframeMaterial} "    
        draw_Text(doc, x1 , y1 -60 , 30, str(textstr), layer='0')    
        textstr = f"Size : {CD-170} x {str(math.floor((y7-y2)*10)/10)}mm"    
        draw_Text(doc, x1 , y1 - 120 , 30, str(textstr), layer='0')        
        textstr = f"Quantity : {su*2} EA"    
        draw_Text(doc, x1 , y1 - 180 , 30, str(textstr), layer='0')         
    ...
    if program:
        ...
        BasicXscale = 2813
        BasicYscale = 1765
        TargetXscale = 2940 + (CW - 1500)  # 기본 모형이 1450
        TargetYscale = 1800 
        if(TargetXscale/BasicXscale > TargetYscale/BasicYscale ):
            frame_scale = TargetXscale/BasicXscale
        else:
        ...
        #11파이, 20파이 홀
        draw_circle(doc, x6 + (CW-226.8)/2, y6 - 15, 11, layer="레이져") 
    ...
    # }        
        ...
        for i, x in enumerate(Bottomx[1:]):       
            circle_cross(doc, temp_x + x-37.4, temp_y + 10, 9, layer='레이져')                
            # print(f"x{i} : {x}")
            if i == 0:
                dim(doc, x1+x-37.4 , y1 , x1, y1 , 100,  direction="down", option='reverse')    
            else:
                dimcontinue(doc , x1 + x-37.4, y1)
        
        for i, x in enumerate(Bottomx[:-1]):       
            circle_cross(doc, temp_x + x-37.4 + CW/2-75, temp_y + 10, 9, layer='레이져')    
            # print(f"x{i} : {x}")
            if i == 0:
                dim(doc, x1+x-37.4 + CW/2-75 , y1 , x1+x-37.4 + CW/2-75-26, y1 , 100,  direction="down", option='reverse')    
            else:
                dimcontinue(doc , x1 + x-37.4 + CW/2-75, y1)
        dimcontinue(doc ,x2, y2 )

        # for i, x in enumerate(Bottomx):       
        #     circle_cross(doc, temp_x + CW/2 + x -75-35.4, temp_y + 10, 9, layer='레이져')    
        #     # print(f"x{i} : {x}")
        #     if i == 0:
        #         dim(doc, x1 + CW/2 + x -75, y1 , x1 + CW/2 + x -75-26, y1 , 100,  direction="down", option='reverse')    
        #     else:
        #         dimcontinue(doc ,x1 + CW/2 + x -75, y1 )
        # dimcontinue(doc ,x2, y2 )
        ...
        # for i, x in enumerate(Bottomx):   
        #     circle_cross(doc, temp_x + x - 35.4, temp_y + 10, 9, layer='레이져')            
        #     if i!= 0:
        
        # for i, x in enumerate(Bottomx):  
        #     if i!= 3:                         
        #         circle_cross(doc, temp_x + CW/2 + x -107-3.4 , temp_y + 10, 9, layer='레이져')    
            

        # 상부 전체치수
        dim(doc, x6+(CW-226.8)/2, y6-15, x6, y6, 100, direction="up", option='reverse') 
        dimcontinue(doc, x5, y5)

        dim(doc, x6+41.6, y1+54, x6, y6, 200, direction="up", option='reverse') 
        dimcontinue(doc, x5-41.6, y1+54)
        dimcontinue(doc, x5, y5)
        
        dim(doc, x6, y6, x5, y5, 170, direction="up") 
        
        
        dim_leader_line(doc, x6 + (CW-226.8)/2, y6-15, x8 + (CW-226.8)/2 + 50 ,y6+ 30, f"%%C11 Holes (M8 Pop Nut)")       
        dim(doc, x6 + (CW-226.8)/2, y6 , x6 + (CW-226.8)/2 , y6-15, 100, direction="left")        
            
        dim_leader_line(doc, x1 + 41.6, y1 + 54, x1+41.6+30 ,y1+54+ 100, f"2-%%C20 Holes")       
        dim(doc, x1, y1 , x1+41.6 , y1+54, 100, direction="right")        
        
        
        # 오른쪽 치수
        dim(doc,  x3, y3 , x2 , y2 , 80,  direction="right")           
        dim(doc,  x4, y4 , x3 , y3 , 80,  direction="right")         
        dim(doc,  x4, y4 , x5 , y5 , 80,  direction="right")         

        # 오른쪽 전체치수
        dim(doc,  x5, y5 , x2 , y2 , 150,  direction="right")          
        ...
        textstr = f"Mat.Spec : {LCframeMaterial}"    
        draw_Text(doc, x1 , y1 -60 , 30, str(textstr), layer='0')    
        textstr = f"Size : {LCW+0.69} x {str(math.floor((y8-y2)*10)/10)}mm"    
        draw_Text(doc, x1 , y1 - 120 , 30, str(textstr), layer='0')        
        textstr = f"Quantity : {su*2} EA"    
        draw_Text(doc, x1 , y1 - 180 , 30, str(textstr), layer='0')    
    ...
    if program:         
        ...
        def draw_arc(doc, start, end, radius):
            """라인을 그리고, 호를 추가하는 함수"""
        ...
        for i, x in enumerate(Bottomx[1:]):       
            circle_cross(doc, temp_x + x+3, temp_y + 10, 9, layer='레이져')    
            # print(f"x{i} : {x}")
            if i == 0:
                dim(doc, x1+x+3 , y1 , x1, y1 , 100,  direction="down", option='reverse')    
            else:
                dimcontinue(doc , x1 + x+3, y1)
        ...
        for i, x in enumerate(linex):
            ...
            if i == 0:
                dim(doc, temp_x + x , temp_y , temp_x, temp_y , 80,  direction="up", option='reverse')    
            else:
        ...
        for i, y in enumerate(liney):
            ...
            if i == 0:
                dim(doc, temp_x - 5 , temp_y + y - 1.5, temp_x - 5, temp_y , 100,  direction="right", option='reverse')    
            else:
        ...
        for i, y in enumerate(liney):
            circle_cross(doc, temp_x - 12, temp_y + y, 9, layer='레이져')    
            if i == 0:
                dim(doc, temp_x , y1 + y , x1, y1 , 100,  direction="left", option='reverse')    
            else:
                dimcontinue(doc , x1, y1 + y)
    ...
    if program:    
        ...
        startx = rx1 + leftwing1
        starty = ry1 + 340 + lc_height + leftwing2*3
        ...
        # 중판 #1 description    
        x1 = frameXpos + 600 + (lc_width-960)
        # y1 = frameYpos + 1100 +  (lc_height-722)
    ...
    if program:         
        ...
        x19 = x18 - leftwing1 + 1 - 6 # 중판 앞쪽은 19, 뒷쪽은 25 따임 만나는 자리
        y19 = y18 
        x20 = x19 
    ...
    if program:
        ...
        dim(doc, x,y+10, x5, y5, 50, direction="up")
        ...
        d(doc, x2,y2, x3, y3, 80, direction="left")
        dim(doc, x4,y4, x5, y5, 110, direction="right")
        dim(doc, x8,y8, x5, y5, 100, direction="up")
...
def main():
    ...
    for file_path in xlsm_files:
        ...
        T5_is = sheet.cell(row=12, column=25).value
